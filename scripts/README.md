...
|---|---|
| `verify-matveev-beal.sh` | Current CI / AMS referee surface: 0 `sorry`, `matveev_gap3_lower` is the B≤10^6 integer-gap close, `C1_floor=143186215390`, proved `gap3_A_bounds` and `matveev_product_bound_of_B_le_B0`, CF lemmas in `MatveevLLL.lean`, displayed-lattice and uniform-`C` no-go in `MatveevBugeaud.lean`, intermediate-`T` no-go in `WuestholzSubgroup.lean`, full-order product packaging in `WuestholzProduct.lean`, log-method no-go in `MatveevThreeLogs.lean` (`bugeaud_LLL_reduction_proof` / `wuestholz_subgroup_theorem` / `wuestholz_product_theorem_exp` / `baker_davenport_reduction` / `baker_bound_gap3` stay `def Prop`), concept DOI `22379293`, slug `beal-level-26-foundations` |
