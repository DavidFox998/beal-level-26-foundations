...
/-!
# Gap-12 `A⁴ + B⁴ = (B+12)¹³` (not a `B ≤ 10⁶` close)

Successor of `5d28874` (`v24-v24x-final-gap11-nogo`).
`C = B+12` so `gcd(B,C) ∣ 12`. Parity is gap-2/4/6/8, not
gap-1/5/7/9/11: `A` is even (odd `A` makes the left side `1`
or `2` mod 16 while even `C` is `0` and odd `C` is odd, never
...
`(1,0)` is **not** a solution (`1 ≠ 12¹³`). Like gap-3
`f1cb039` and gap-6 `8425abd`, `3 ∣ B` is a 3-descent:
`A=3A₁ B=3B₁ C=3(B₁+4)` gives `A₁⁴+B₁⁴=3⁹ C₁¹³`, and after
two more steps `27 X = C₁¹³` while `C₁=B₁+4 ≡ 1 mod 3`.
`5 ∣ B` kills residue (`C ≡ 2 mod 5`, `C¹³ ≡ 2`, not in
`{0,1}`). `7 ∣ B` kills residue (`C ≡ 5 mod 7`, `C¹³ ≡ 5`,
not in `{0,1,2,4}`). Remaining solutions are coprime
(`p ∣ A,B` ⇒ `p ∣ 12`, but `B` is odd and `3 ∤ B`) and
pure Darmon–Merel signature `(4,4,13)`.

`hGen_gap12` / `hLLL_gap12` stay `def Prop`. Every `C1 ≥ 1`
loses to `B⁴/A⁴` on `B ≥ B0`; no scaling yields a
Baker–Davenport witness above `17 + C/B0`. Named theorem
`baker_bound_gap12` stays a `def Prop`. 0 sorry. No new axiom.
...
def is_gap12_sol (A B : ℕ) : Prop :=
  A ^ 4 + B ^ 4 = (B + 12) ^ 13

/-- Kernel Baker bound for gap-12. Stays a `def Prop`. -/
def baker_bound_gap12 : Prop :=
  ∀ A B : ℕ, is_gap12_sol A B → B ≤ B0_nat
...
theorem not_gap12_one_zero : ¬ is_gap12_sol 1 0 := by
  unfold is_gap12_sol
  decide

theorem even_twelve : Even (12 : ℕ) := ⟨6, rfl⟩

/-! ## `gcd(B, B+12) = gcd(B, 12)` divides `12` -/

theorem gcd_B_C_eq_gcd_twelve (B : ℕ) :
    Nat.gcd B (B + 12) = Nat.gcd B 12 := by
  rw [Nat.gcd_comm B (B + 12), Nat.add_comm B 12]
  exact (Nat.gcd_add_self_left (12 : ℕ) B).trans (Nat.gcd_comm 12 B)

theorem gcd_B_C_dvd_twelve (B : ℕ) : Nat.gcd B (B + 12) ∣ 12 := by
  rw [gcd_B_C_eq_gcd_twelve]
  exact Nat.gcd_dvd_right B 12

theorem gcd_B_C_dvd_twelve_gap12 (B : ℕ) : Nat.gcd B (B + 12) ∣ 12 :=
  gcd_B_C_dvd_twelve B

theorem odd_of_sixteen_mul_add_three (n : ℕ) : Odd (16 * n + 3) := by
  rw [Nat.odd_iff, Nat.add_mod, Nat.mul_mod]
  norm_num

/-! ## Parity: `A` is even. Even `B` is impossible, so `B` is odd. -/

theorem A_even_of_sol_gap12 {A B : ℕ} (h : is_gap12_sol A B) : Even A := by
  by_contra hne
  have hAodd : Odd A := Nat.not_even_iff_odd.1 hne
  have hA4 : A ^ 4 % 16 = 1 :=
    BealCatalanZsig.odd_pow_four_mod_sixteen hAodd
  by_cases hBeven : Even B
  · have hCeven : Even (B + 12) := hBeven.add even_twelve
    have h16C : 16 ∣ (B + 12) ^ 4 :=
      BealCatalanZsig.even_pow_four_dvd_sixteen hCeven
    have h16R : 16 ∣ (B + 12) ^ 13 :=
      dvd_trans h16C (pow_dvd_pow (B + 12) (by decide : 4 ≤ 13))
    have hR : (B + 12) ^ 13 % 16 = 0 := Nat.mod_eq_zero_of_dvd h16R
    have hB4 : B ^ 4 % 16 = 0 :=
      Nat.mod_eq_zero_of_dvd (BealCatalanZsig.even_pow_four_dvd_sixteen hBeven)
    have hL : (A ^ 4 + B ^ 4) % 16 = 1 := by
      rw [Nat.add_mod, hA4, hB4]
    have : (A ^ 4 + B ^ 4) % 16 = (B + 12) ^ 13 % 16 := by rw [h]
    rw [hL, hR] at this
    exact (by decide : (1 : ℕ) ≠ 0) this
  · have hBodd : Odd B := Nat.not_even_iff_odd.1 hBeven
    have hCodd : Odd (B + 12) := hBodd.add_even even_twelve
    have hB4 : B ^ 4 % 16 = 1 :=
      BealCatalanZsig.odd_pow_four_mod_sixteen hBodd
    have hL : (A ^ 4 + B ^ 4) % 16 = 2 := by
      rw [Nat.add_mod, hA4, hB4]
    have hR : (B + 12) ^ 13 % 16 = (B + 12) % 16 :=
      BealMatveevBeal.BealGap2.odd_pow_thirteen_mod_sixteen hCodd
    have : (A ^ 4 + B ^ 4) % 16 = (B + 12) ^ 13 % 16 := by rw [h]
    rw [hL, hR] at this
    have hC2 : (B + 12) % 2 = 0 := by
      have h16 : (B + 12) % 16 = 2 := this.symm
      have : ((B + 12) % 16) % 2 = 0 := by rw [h16]
      rwa [Nat.mod_mod_of_dvd (B + 12) (by decide : 2 ∣ 16)] at this
    exact Nat.not_even_iff_odd.2 hCodd (Nat.even_iff.2 hC2)

/-- Even `B` is impossible: six divisions by `2` leave
    `A₆⁴ + B₆⁴ = 4 (16 B₆ + 3)¹³`, whose residue is `4` or `12`
    mod 16 (here always `12`) and cannot match a sum of fourth
    powers. Do **not** prove `B_even_of_sol_gap12`. -/
theorem no_sol_when_even_B {A B : ℕ}
    (h : is_gap12_sol A B) (hBeven : Even B) : False := by
  have hAeven := A_even_of_sol_gap12 h
  obtain ⟨A1, hA'⟩ := hAeven
  obtain ⟨B1, hB'⟩ := hBeven
  have hA : A = 2 * A1 := by rw [hA', two_mul]
  have hB : B = 2 * B1 := by rw [hB', two_mul]
  have hC : B + 12 = 2 * (B1 + 6) := by
    rw [hB, mul_add]
  have hsol1 : (2 * A1) ^ 4 + (2 * B1) ^ 4 = (2 * (B1 + 6)) ^ 13 := by
    rw [← hA, ← hB, ← hC]; exact h
  have heq1 : 16 * (A1 ^ 4 + B1 ^ 4) = 2 ^ 13 * (B1 + 6) ^ 13 := by
    rw [← BealMatveevBeal.BealGap2.sum_fourth_of_mul_two, hsol1, mul_pow]
  have h9 : A1 ^ 4 + B1 ^ 4 = 2 ^ 9 * (B1 + 6) ^ 13 := by
  ...
  have h18 : A2 ^ 4 + B2 ^ 4 = 2 ^ 18 * (B2 + 3) ^ 13 := by
    have hr : (2 * (B2 + 3)) ^ 13 = 2 ^ 13 * (B2 + 3) ^ 13 :=
      mul_pow _ _ 13
    apply Nat.eq_of_mul_eq_mul_left (by decide : 0 < 16)
    rw [heq2, hr]
  ...
  have heq6src : (2 * A6) ^ 4 + (2 * B6) ^ 4 =
      2 ^ 6 * (16 * B6 + 3) ^ 13 := by
    have hB21' : 8 * B5 + 3 = 16 * B6 + 3 := by
      rw [hB6]
      have hmul : 8 * (2 * B6) = 16 * B6 := by
        rw [← mul_assoc]; rfl
      rw [hmul]
    rw [← hA6, ← hB6, ← hB21']; exact h6
  have heq6 : 16 * (A6 ^ 4 + B6 ^ 4) = 2 ^ 6 * (16 * B6 + 3) ^ 13 := by
    rw [← BealMatveevBeal.BealGap2.sum_fourth_of_mul_two]; exact heq6src
  have hlast : A6 ^ 4 + B6 ^ 4 = 4 * (16 * B6 + 3) ^ 13 := by
    have hpow : (2 : ℕ) ^ 6 = 16 * 4 := by norm_num
    apply Nat.eq_of_mul_eq_mul_left (by decide : 0 < 16)
    rw [heq6, hpow, mul_assoc]
  have hodd : Odd ((16 * B6 + 3) ^ 13) :=
    Odd.pow (odd_of_sixteen_mul_add_three B6)
  exact BealMatveevBeal.BealGap4.not_sum_fourth_eq_four_mul_odd hodd hlast

theorem B_odd_of_sol_gap12 {A B : ℕ} (h : is_gap12_sol A B) : Odd B := by
...
theorem two_mul_B_pow_four_lt_add_twelve_pow (B : ℕ) :
    2 * B ^ 4 < (B + 12) ^ 13 := by
  have h12 : 12 ≤ B + 12 := Nat.le_add_left 12 B
  have h2 : 2 ≤ B + 12 := le_trans (by decide : 2 ≤ 12) h12
  have h2pow : 2 ≤ 2 ^ 9 := by decide
  have h29 : 2 ^ 9 ≤ (B + 12) ^ 9 := Nat.pow_le_pow_left h2 9
  have h2le : 2 ≤ (B + 12) ^ 9 := le_trans h2pow h29
  have hBlt : B < B + 12 :=
    Nat.lt_add_of_pos_right (by decide : 0 < (12 : ℕ))
  have hB4 : B ^ 4 < (B + 12) ^ 4 :=
    Nat.pow_lt_pow_left hBlt (by decide : 4 ≠ 0)
  have hle : 2 * B ^ 4 ≤ (B + 12) ^ 9 * B ^ 4 :=
    Nat.mul_le_mul_right (B ^ 4) h2le
  have hposC : 0 < B + 12 := Nat.add_pos_right B (by decide : 0 < 12)
  have hpos9 : 0 < (B + 12) ^ 9 := Nat.pos_pow_of_pos 9 hposC
  have hlt : (B + 12) ^ 9 * B ^ 4 < (B + 12) ^ 9 * (B + 12) ^ 4 :=
    Nat.mul_lt_mul_of_pos_left hB4 hpos9
  have hsum : (B + 12) ^ 9 * (B + 12) ^ 4 = (B + 12) ^ 13 := by
    rw [← pow_add]
  rw [← hsum]
  exact lt_of_le_of_lt hle hlt

theorem A_gt_B_of_sol {A B : ℕ}
    (h : is_gap12_sol A B) (_hB : 0 < B) : B < A := by
  by_contra hle
  ...
  have hlt := two_mul_B_pow_four_lt_add_twelve_pow B
  have : A ^ 4 + B ^ 4 < (B + 12) ^ 13 := lt_of_le_of_lt hsum hlt
  exact (ne_of_lt this) h

theorem A_ne_zero_of_sol {A B : ℕ} (h : is_gap12_sol A B) : A ≠ 0 := by
  intro hA
  subst hA
  have heq : B ^ 4 = (B + 12) ^ 13 := by
    simpa [is_gap12_sol] using h
  by_cases hB : B = 0
  · subst hB
    exact (by decide : (0 : ℕ) ≠ 106993205379072) heq
  · have hpos : 0 < B := Nat.pos_of_ne_zero hB
    ...
    have hlt1 : B ^ 4 < 2 * B ^ 4 := by
      ...
      simpa using this
    have hlt2 := two_mul_B_pow_four_lt_add_twelve_pow B
    exact (ne_of_lt (lt_trans hlt1 hlt2)) heq

theorem A_gt_one_of_sol_pos {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) : 1 < A :=
...
/-- `C = B+12 ≡ 0 mod 3`, so `C¹³ ≡ 0` and `A⁴ ≡ 0`, hence `3 ∣ A`. -/
theorem three_dvd_B_forces_three_dvd_A_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (h3 : 3 ∣ B) : 3 ∣ A := by
  have hB4 : B ^ 4 % 3 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h3 (by decide))
  have hC : (B + 12) % 3 = 0 := by
    have : B % 3 = 0 := Nat.mod_eq_zero_of_dvd h3
    rw [Nat.add_mod, this]
  have hC13 : (B + 12) ^ 13 % 3 = 0 := by
    rw [pow13_mod3, hC]
  have hsum : (A ^ 4 + B ^ 4) % 3 = (B + 12) ^ 13 % 3 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
  exact three_dvd_of_fourth_pow_mod_zero hsum

/-- `3 ∣ B` is impossible: `C₁ = B₁+4` stays `≡ 1` after further
    `/3`, then `27 X = (B₁+4)¹³` while `B₁+4 ≡ 1 mod 3`. -/
theorem no_sol_when_three_dvd_B_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (h3 : 3 ∣ B) : False := by
  have hA3 := three_dvd_B_forces_three_dvd_A_gap12 h h3
  obtain ⟨A1, hA⟩ := hA3
  obtain ⟨B1, hB⟩ := h3
  have hC : B + 12 = 3 * (B1 + 4) := by
    rw [hB, mul_add]
  have hsol1 : (3 * A1) ^ 4 + (3 * B1) ^ 4 = (3 * (B1 + 4)) ^ 13 := by
    rw [← hA, ← hB, ← hC]; exact h
  have heq1 : 81 * (A1 ^ 4 + B1 ^ 4) = 3 ^ 13 * (B1 + 4) ^ 13 := by
    rw [← sum_fourth_of_mul_three, hsol1, mul_pow]
  have h9 : A1 ^ 4 + B1 ^ 4 = 3 ^ 9 * (B1 + 4) ^ 13 := by
  ...
  have h5eq : A2 ^ 4 + B2 ^ 4 = 3 ^ 5 * (B1 + 4) ^ 13 := by
  ...
  have h1eq : A3 ^ 4 + B3 ^ 4 = 3 * (B1 + 4) ^ 13 := by
    have hpow : (3 : ℕ) ^ 5 = 81 * 3 := by norm_num
    apply Nat.eq_of_mul_eq_mul_left (by decide : 0 < 81)
    rw [heq3, hpow, mul_assoc]
  have hmod3 : (A3 ^ 4 + B3 ^ 4) % 3 = 0 := by
    have : 3 ∣ 3 * (B1 + 4) ^ 13 := dvd_mul_right _ _
  ...
  have heq4src : (3 * A4) ^ 4 + (3 * B4) ^ 4 =
    ...
    rw [← hA4, ← hB4]
    exact h1eq
  have heq4 : 81 * (A4 ^ 4 + B4 ^ 4) = 3 * (B1 + 4) ^ 13 := by
    rw [← sum_fourth_of_mul_three]; exact heq4src
  have h27 : 27 * (A4 ^ 4 + B4 ^ 4) = (B1 + 4) ^ 13 := by
  ...
  have hC13 : 3 ∣ (B1 + 4) ^ 13 :=
    ⟨9 * (A4 ^ 4 + B4 ^ 4), by
      rw [← h27, ← mul_assoc]
      rfl⟩
  have hC1 : 3 ∣ B1 + 4 := Nat.Prime.dvd_of_dvd_pow Nat.prime_three hC13
  have hB1three : 3 ∣ B1 := ⟨B2, hB2⟩
  have hCmod : (B1 + 4) % 3 = 1 := by
    have : B1 % 3 = 0 := Nat.mod_eq_zero_of_dvd hB1three
    rw [Nat.add_mod, this]
  have hC0 : (B1 + 4) % 3 = 0 := Nat.mod_eq_zero_of_dvd hC1
  rw [hCmod] at hC0
  exact (by decide : (1 : ℕ) ≠ 0) hC0

theorem three_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap12_sol A B) : ¬ 3 ∣ B :=
  fun h3 => no_sol_when_three_dvd_B_gap12 h h3

/-! ## Coprimeness: `gcd(A,B) = 1` on every solution -/

theorem gcd_A_B_eq_one_of_sol {A B : ℕ}
    (h : is_gap12_sol A B) : Nat.gcd A B = 1 := by
  have hBodd := B_odd_of_sol_gap12 h
  have h3n := three_not_dvd_B_of_sol h
  set d := Nat.gcd A B
  have hdA : d ∣ A := Nat.gcd_dvd_left A B
  have hdB : d ∣ B := Nat.gcd_dvd_right A B
  have hdC13 : d ∣ (B + 12) ^ 13 := by
    have hA4 : d ∣ A ^ 4 := dvd_pow hdA (by decide)
    have hB4 : d ∣ B ^ 4 := dvd_pow hdB (by decide)
    have hsum : d ∣ A ^ 4 + B ^ 4 := Nat.dvd_add hA4 hB4
    simpa [is_gap12_sol] using (h ▸ hsum)
  have hgcd12 : Nat.gcd d (B + 12) ∣ 12 := by
    have h1 : Nat.gcd d (B + 12) ∣ B :=
      dvd_trans (Nat.gcd_dvd_left d (B + 12)) hdB
    have h2 : Nat.gcd d (B + 12) ∣ (B + 12) := Nat.gcd_dvd_right d (B + 12)
    exact dvd_trans (Nat.dvd_gcd h1 h2) (gcd_B_C_dvd_twelve B)
  have hgcd1 : Nat.gcd d (B + 12) = 1 := by
    by_contra hne
    have hpos : 1 < Nat.gcd d (B + 12) :=
      Nat.lt_of_le_of_ne (Nat.succ_le_of_lt
        (Nat.gcd_pos_of_pos_right d
          (Nat.add_pos_right B (by decide : 0 < 12))))
        (Ne.symm hne)
    obtain ⟨p, hp, hpd⟩ := Nat.exists_prime_and_dvd (ne_of_gt hpos)
    have hp12 : p ∣ 12 := dvd_trans hpd hgcd12
    have hp23 : p = 2 ∨ p = 3 := by
      have hmul : p ∣ 4 ∨ p ∣ 3 :=
        (Nat.Prime.dvd_mul hp).mp (by simpa using hp12)
      rcases hmul with h4 | h3p
      · have h2 : p ∣ 2 :=
          Nat.Prime.dvd_of_dvd_pow hp (by simpa using h4 : p ∣ 2 ^ 2)
        exact Or.inl
  ...
  have hcop : Nat.Coprime d (B + 12) := hgcd1
  have hcop13 : Nat.Coprime d ((B + 12) ^ 13) := hcop.pow_right 13
  have hd1 : d ∣ 1 := by
    have := Nat.dvd_gcd (dvd_refl d) hdC13
    rwa [show Nat.gcd d ((B + 12) ^ 13) = 1 from hcop13] at this
  exact Nat.eq_one_of_dvd_one hd1

theorem gcd_A_B_eq_one_of_sol_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) : Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

theorem coprime_of_sol_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) : Nat.Coprime A B :=
  gcd_A_B_eq_one_of_sol h

theorem gcd_A_B_eq_one_of_remaining_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (_h3 : ¬ 3 ∣ B) (_h5 : ¬ 5 ∣ B)
    (_h7 : ¬ 7 ∣ B) : Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

theorem coprime_of_remaining_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (h3 : ¬ 3 ∣ B) (h5 : ¬ 5 ∣ B)
    (h7 : ¬ 7 ∣ B) : Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_remaining_gap12 h h3 h5 h7
...
/-- `C = B+12 ≡ 2 mod 5`, so `C¹³ ≡ 2`, while `A⁴ + B⁴ ≡ A⁴ ∈ {0,1}`. -/
theorem no_sol_when_five_dvd_B_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (h5 : 5 ∣ B) : False := by
  have hB4 : B ^ 4 % 5 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h5 (by decide))
  have hC : (B + 12) % 5 = 2 := by
    have : B % 5 = 0 := Nat.mod_eq_zero_of_dvd h5
    rw [Nat.add_mod, this]
  have hC13 : (B + 12) ^ 13 % 5 = 2 := by
    rw [pow13_mod5, hC]
  have hsum : (A ^ 4 + B ^ 4) % 5 = (B + 12) ^ 13 % 5 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
...
theorem five_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap12_sol A B) : ¬ 5 ∣ B :=
  fun h5 => no_sol_when_five_dvd_B_gap12 h h5

/-! ## `mod 7`: `7 ∣ B` is a contradiction (`C ≡ 5`) -/
...
/-- `C = B+12 ≡ 5 mod 7`, so `C¹³ ≡ 5`, not in `{0,1,2,4}`. -/
theorem no_sol_when_seven_dvd_B_gap12 {A B : ℕ}
    (h : is_gap12_sol A B) (h7 : 7 ∣ B) : False := by
  have hB4 : B ^ 4 % 7 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h7 (by decide))
  have hC : (B + 12) % 7 = 5 := by
    have : B % 7 = 0 := Nat.mod_eq_zero_of_dvd h7
    rw [Nat.add_mod, this]
  have hC13 : (B + 12) ^ 13 % 7 = 5 := by
    rw [pow13_mod7, hC]
  have hsum : (A ^ 4 + B ^ 4) % 7 = (B + 12) ^ 13 % 7 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
...
theorem seven_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap12_sol A B) : ¬ 7 ∣ B :=
  fun h7 => no_sol_when_seven_dvd_B_gap12 h h7

/-! ## Linear form `Λ = 4 log A − 13 log(B+12)` -/

noncomputable def Lambda_gap12 (A B : ℕ) : ℝ :=
  4 * log (A : ℝ) - 13 * log ((B + 12 : ℕ) : ℝ)

theorem A_pow_four_eq_sub {A B : ℕ} (h : is_gap12_sol A B) :
    A ^ 4 = (B + 12) ^ 13 - B ^ 4 := by
  rw [← h, add_comm, Nat.add_sub_cancel_left]

theorem A_pow_real_add {A B : ℕ} (h : is_gap12_sol A B) :
    (A : ℝ) ^ 4 + (B : ℝ) ^ 4 = ((B + 12 : ℕ) : ℝ) ^ 13 := by
  have h' : ((A ^ 4 + B ^ 4 : ℕ) : ℝ) = (((B + 12) ^ 13 : ℕ) : ℝ) :=
    congrArg (fun n : ℕ => (n : ℝ)) h
  simpa [Nat.cast_add, Nat.cast_pow] using h'

theorem A_pow_pos_of_sol {A B : ℕ} (h : is_gap12_sol A B) :
    (0 : ℝ) < (A : ℝ) ^ 4 := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  exact pow_pos hApos 4

theorem Lambda_gap12_eq_log_div {A B : ℕ}
    (h : is_gap12_sol A B) :
    Lambda_gap12 A B =
      log ((A : ℝ) ^ 4 / ((B + 12 : ℕ) : ℝ) ^ 13) := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  have hCpos : (0 : ℝ) < ((B + 12 : ℕ) : ℝ) := by
    have : 0 < B + 12 := Nat.add_pos_right B (by decide : 0 < 12)
    exact_mod_cast this
  unfold Lambda_gap12
  have h4 : (4 : ℝ) * log (A : ℝ) = log ((A : ℝ) ^ 4) :=
    (Real.log_pow (A : ℝ) 4).symm
  have h13 : (13 : ℝ) * log ((B + 12 : ℕ) : ℝ) =
      log (((B + 12 : ℕ) : ℝ) ^ 13) :=
    (Real.log_pow ((B + 12 : ℕ) : ℝ) 13).symm
  rw [h4, h13, Real.log_div (pow_pos hApos 4).ne' (pow_pos hCpos 13).ne']

theorem abs_Lambda_gap12_eq_log_one_plus {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) :
    |Lambda_gap12 A B| = log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) := by
  ...
  have hΛ := Lambda_gap12_eq_log_div h
...
theorem abs_Lambda_gap12_le_ratio {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) :
    |Lambda_gap12 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  have hA4pos := A_pow_pos_of_sol h
  have hBpos : (0 : ℝ) < (B : ℝ) := by exact_mod_cast hB
  have hu : (0 : ℝ) < (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
    div_pos (pow_pos hBpos 4) hA4pos
  ...
  have : log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ≤
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
    simpa using hlog
  rwa [abs_Lambda_gap12_eq_log_one_plus h hB]

theorem B_pow_five_add_B_pow_four_lt_add_twelve_pow {B : ℕ} (_hB : 0 < B) :
    (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 12 : ℕ) : ℝ) ^ 13 := by
  have hx : (0 : ℝ) ≤ (B : ℝ) := Nat.cast_nonneg B
  have hC : ((B + 12 : ℕ) : ℝ) = (B : ℝ) + 12 := by
    rw [Nat.cast_add]
    norm_num
  have h5 : (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 12 : ℕ) : ℝ) ^ 5 := by
    rw [hC]
    have hexp : ((B : ℝ) + 12) ^ 5 =
        (B : ℝ) ^ 5 + 60 * (B : ℝ) ^ 4 + 1440 * (B : ℝ) ^ 3 +
          17280 * (B : ℝ) ^ 2 + 103680 * (B : ℝ) + 248832 := by
      ring
    have h3 : (0 : ℝ) ≤ (B : ℝ) ^ 3 := pow_nonneg hx 3
    have h2 : (0 : ℝ) ≤ (B : ℝ) ^ 2 := sq_nonneg _
    nlinarith [hexp, h3, h2, hx]
  have hC1 : (1 : ℝ) ≤ ((B + 12 : ℕ) : ℝ) := by
    have : 1 ≤ B + 12 :=
      le_trans (by decide : 1 ≤ 12) (Nat.le_add_left 12 B)
    exact_mod_cast this
  have h513 : ((B + 12 : ℕ) : ℝ) ^ 5 ≤ ((B + 12 : ℕ) : ℝ) ^ 13 :=
...
theorem A_pow_gt_B_pow_five {A B : ℕ}
    ...
    (B : ℝ) ^ 5 < (A : ℝ) ^ 4 := by
  have hA4 : (A : ℝ) ^ 4 =
      ((B + 12 : ℕ) : ℝ) ^ 13 - (B : ℝ) ^ 4 := by
    linarith [A_pow_real_add h]
  have hsum := B_pow_five_add_B_pow_four_lt_add_twelve_pow hB
  linarith [hA4, hsum]
...
theorem abs_Lambda_gap12_lt_inv_B {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) :
    |Lambda_gap12 A B| < 1 / (B : ℝ) :=
  lt_of_le_of_lt (abs_Lambda_gap12_le_ratio h hB) (ratio_lt_inv_B h hB)

theorem A_pow_lt_C_pow {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) :
    (A : ℝ) ^ 4 < ((B + 12 : ℕ) : ℝ) ^ 13 := by
  have hB4 : 0 < B ^ 4 := Nat.pos_pow_of_pos 4 hB
  have hlt : A ^ 4 < (B + 12) ^ 13 :=
    calc
...
theorem logA_gt_thirteen_of_B_ge_B0 {A B : ℕ}
    (h : is_gap12_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (13 : ℝ) < log (A : ℝ) := by
...
/-- `exp(−C1 log A log(B+12)) < B⁴/A⁴` for every `C1 ≥ 1` on
    a gap-12 solution with `B ≥ B0`. Same cutoff as `62383af`:
    `C1 < 1` would be needed. -/
theorem hGen_exp_lt_ratio_gap12
    {C1 : ℝ} (hC1 : 1 ≤ C1)
    {A B : ℕ} (hsol : is_gap12_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-C1 * log (A : ℝ) * log ((B + 12 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  set a : ℝ := log (A : ℝ)
  set c : ℝ := log ((B + 12 : ℕ) : ℝ)
  set C : ℝ := ((B + 12 : ℕ) : ℝ)
  have hAgt1 : 1 < A := A_gt_one_of_sol_pos hsol hB
  have hCgt1 : 1 < B + 12 :=
    lt_of_lt_of_le (by decide : 1 < 12) (Nat.le_add_left 12 B)
  have hApos : (0 : ℝ) < (A : ℝ) := by
    have : (0 : ℕ) < A := lt_trans Nat.zero_lt_one hAgt1
    exact_mod_cast this
  have hCpos : (0 : ℝ) < C := by
    have : 0 < B + 12 := Nat.add_pos_right B (by decide : 0 < 12)
    have : (0 : ℝ) < ((B + 12 : ℕ) : ℝ) := by exact_mod_cast this
    simpa [C] using this
  have hCgt : (1 : ℝ) < C := by
    have : (1 : ℝ) < ((B + 12 : ℕ) : ℝ) := by exact_mod_cast hCgt1
    simpa [C] using this
...
theorem C1_floor_hGen_loses_gap12 {A B : ℕ}
    (hsol : is_gap12_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-(C1_floor : ℝ) * log (A : ℝ) * log ((B + 12 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
  hGen_exp_lt_ratio_gap12
    (le_of_lt (by
      simpa [C1_floor_eq_logs] using
        BealMatveevBeal.MatveevThreeLogs.C1_floor_gt_one))
    hsol hB hB0

/-! ## Floor form / LLL no-go (`B+12`) -/

noncomputable def floorFormThird_gap12 (C : ℝ) (A B : ℕ) : ℝ :=
  (4 : ℝ) * ⌊C * log (A : ℝ)⌋ - 13 * ⌊C * log ((B + 12 : ℕ) : ℝ)⌋

/-- `|4⌊C log A⌋ − 13⌊C log(B+12)⌋ − C Λ| < 17` for every real `C`. -/
theorem floor_form_approx_of_C_gap12 (C : ℝ) (A B : ℕ) :
    |floorFormThird_gap12 C A B - C * Lambda_gap12 A B| < 17 := by
  set a : ℝ := C * log (A : ℝ)
  set cval : ℝ := C * log ((B + 12 : ℕ) : ℝ)
  have hΛ : C * Lambda_gap12 A B = 4 * a - 13 * cval := by
    simp only [Lambda_gap12, a, cval]
    ring
  have hft : floorFormThird_gap12 C A B = (4 : ℝ) * ⌊a⌋ - 13 * ⌊cval⌋ := rfl
  have hform :
      floorFormThird_gap12 C A B - C * Lambda_gap12 A B =
...
theorem floor_form_third_abs_lt_gap12 {C : ℝ} (hC : 0 < C) (A B : ℕ) :
    |floorFormThird_gap12 C A B| < 17 + C * |Lambda_gap12 A B| := by
  have happ := floor_form_approx_of_C_gap12 C A B
  have hmul : |C * Lambda_gap12 A B| = C * |Lambda_gap12 A B| := by
...
/-- No scaling `C > 0` yields a Baker–Davenport witness
    `r > 17 + C/B0` on a gap-12 solution with `B ≥ B0`. -/
theorem hLLL_method_fails_gap12 :
    ∀ C : ℝ, 0 < C →
      ∀ A B : ℕ,
        is_gap12_sol A B →
          0 < B →
            B0_nat ≤ B →
              ¬ ∃ r : ℝ,
                17 + C / (B0_nat : ℝ) < r ∧
                  r ≤ |floorFormThird_gap12 C A B| := by
  intro C hC A B hsol hB hB0
  rintro ⟨r, hgt, hle⟩
  have htri := floor_form_third_abs_lt_gap12 hC A B
  have hLam := abs_Lambda_gap12_lt_inv_B hsol hB
  have hB0pos : (0 : ℝ) < (B0_nat : ℝ) := by
    have : (0 : ℕ) < B0_nat := by decide
    exact_mod_cast this
  have hBle : (B0_nat : ℝ) ≤ (B : ℝ) := Nat.cast_le.mpr hB0
  have hCΛ : C * |Lambda_gap12 A B| < C / (B0_nat : ℝ) := by
    have hmid : C * |Lambda_gap12 A B| < C * (1 / (B : ℝ)) :=
  ...
  have hthird : |floorFormThird_gap12 C A B| < 17 + C / (B0_nat : ℝ) := by
    linarith [htri, hCΛ]
  linarith [hgt, hle, hthird]

theorem no_uniform_c_cubic_in_N_gap12 :
    ¬ ∃ c : ℝ, 0 < c ∧
      ∀ L N1 N2 N3 : ℕ,
...
theorem coeffCount3_zero_ten_ten_ten_gap12 :
...
theorem no_uniform_c_easy_half_gap12 :
    (1 / 2 : ℝ) * 1 * 1 < 242 := by
  norm_num
...
/-- Matveev 2000 Theorem 1.4, n=2, displayed `C1_floor` exponential
    on a gap-12 solution. Not in Mathlib 4.12. Instantiating it at
...
/-- Bugeaud / LLL close for gap-12: ratio upper bound ⇒ `B ≤ B0`.
    The displayed lattice does not inhabit it. -/
def hLLL_gap12 : Prop :=
  (∀ A B : ℕ, is_gap12_sol A B → 0 < B →
      |Lambda_gap12 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4) →
    baker_bound_gap12

def baker_bound_gap12_of_hGen_hLLL : Prop :=
  hGen_gap12 → hLLL_gap12 → baker_bound_gap12

def baker_bound_gap12_of_hGen_hLLL_gap12 : Prop :=
  baker_bound_gap12_of_hGen_hLLL
...
/-- Log and LLL methods lose on `B ≥ B0`. Remaining solutions are
    ...
    refuted by any such solution. `baker_bound_gap12` stays
    uninhabited as a named theorem. -/
theorem baker_bound_gap12_unconditional_nogo {A B : ℕ}
    (hsol : is_gap12_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (∀ C1 : ℝ, 1 ≤ C1 →
      exp (-C1 * log (A : ℝ) * log ((B + 12 : ℕ) : ℝ)) <
        (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ∧
  ...
  refine ⟨?_, ?_, coprime_of_sol_gap12 hsol, ?_,
    B_odd_of_sol_gap12 hsol, A_even_of_sol_gap12 hsol,
    three_not_dvd_B_of_sol hsol, five_not_dvd_B_of_sol hsol,
    seven_not_dvd_B_of_sol hsol, ?_⟩
  · intro C1 hC1
    exact hGen_exp_lt_ratio_gap12 hC1 hsol hB hB0
  · intro C hC
    exact hLLL_method_fails_gap12 C hC A B hsol hB hB0
  · exact Nat.lt_of_le_of_lt hB0 (A_gt_B_of_sol hsol hB)
  · intro hDM
    have hA0 : A = 0 ∨ B = 0 :=
      hDM A B (B + 12) (coprime_of_sol_gap12 hsol) hsol
    cases hA0 with
...
/-- Every gap-12 solution is impossible relative to
    `darmon_merel_4413_axiom`. Unlike gap-1 there is no `(1,0)`. -/
theorem no_gap12_of_axiom {A B : ℕ}
    (hsol : is_gap12_sol A B) : False := by
...
/-- Vacuous `B ≤ B0` for every gap-12 solution, relative to the
    existing Darmon–Merel axiom. Not named `baker_bound_gap12`. -/
theorem baker_bound_gap12_true {A B : ℕ}
    (hsol : is_gap12_sol A B) : B ≤ B0_nat :=
  (no_gap12_of_axiom hsol).elim

theorem baker_bound_gap12_true_of_axiom {A B : ℕ}
    (hsol : is_gap12_sol A B) : B ≤ B0_nat :=
  baker_bound_gap12_true hsol

#check C1_floor_eq
#check B0_nat_eq
#check not_gap12_one_zero
#check gcd_B_C_eq_gcd_twelve
#check gcd_B_C_dvd_twelve
#check gcd_B_C_dvd_twelve_gap12
#check gcd_A_B_eq_one_of_sol
#check coprime_of_sol_gap12
#check B_odd_of_sol_gap12
#check A_even_of_sol_gap12
#check no_sol_when_even_B
#check A_gt_B_of_sol
#check three_dvd_B_forces_three_dvd_A_gap12
#check no_sol_when_three_dvd_B_gap12
#check no_sol_when_five_dvd_B_gap12
#check no_sol_when_seven_dvd_B_gap12
#check coprime_of_remaining_gap12
#check hGen_exp_lt_ratio_gap12
#check C1_floor_hGen_loses_gap12
#check floor_form_approx_of_C_gap12
#check hLLL_method_fails_gap12
#check no_uniform_c_cubic_in_N_gap12
#check no_uniform_c_easy_half_gap12
#check hGen_gap12
#check hLLL_gap12
#check baker_bound_gap12_of_hGen_hLLL
#check baker_bound_gap12_unconditional_nogo
#check baker_bound_gap12
#check no_gap12_of_axiom
#check baker_bound_gap12_true
#check baker_bound_gap12_true_of_axiom
#print axioms gcd_A_B_eq_one_of_sol
#print axioms B_odd_of_sol_gap12
#print axioms A_even_of_sol_gap12
#print axioms no_sol_when_even_B
#print axioms no_sol_when_three_dvd_B_gap12
#print axioms three_dvd_B_forces_three_dvd_A_gap12
#print axioms no_sol_when_five_dvd_B_gap12
#print axioms no_sol_when_seven_dvd_B_gap12
#print axioms hGen_exp_lt_ratio_gap12
#print axioms C1_floor_hGen_loses_gap12
#print axioms hLLL_method_fails_gap12
#print axioms baker_bound_gap12_unconditional_nogo
#print axioms no_gap12_of_axiom
#print axioms baker_bound_gap12_true

end BealMatveevBeal.BealGap12
