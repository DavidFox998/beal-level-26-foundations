...
/-!
# Gap-14 `A⁴ + B⁴ = (B+14)¹³` (not a `B ≤ 10⁶` close)

Successor of `97e4841` (`v24-v24x-final-gap13-nogo`).
`C = B+14` so `gcd(B,C) ∣ 14`. Parity is gap-2/4/6/8/10/12, not
gap-1/5/7/9/11/13: **`B` is odd** and **`A` is even**. Even `B`
is not an infinite `A₁⁴+B₁⁴=512 C₁¹³` forever (gap-12 lesson):
three divisions by `2` leave `A₃⁴+B₃⁴=2(4B₃+7)¹³`, whose residue
is `6` or `14` mod 16, never a sum of fourth powers `{0,1,2}`.
`(1,0)` is **not** a solution (`1 ≠ 14¹³`). `B` odd forces
...
`14 ≢ 0 mod 5`, so this is **not** a 5-descent). `7 ∣ B` kills
by 7-descent: `A=7A₁ B=7B₁ C=7(B₁+2)` gives `A₁⁴+B₁⁴=7⁹ C₁¹³`
(`7⁹ = 40353607`); fourth powers mod 7 are `{0,1,2,4}` and sum
`0` only `0+0`, so three further `/7` leave `343 X = (B₁+2)¹³`
while `B₁+2 ≡ 2 mod 7`. Remaining solutions are coprime
(`p ∣ A,B` ⇒ `p ∣ 14`, but `B` is odd so `p ≠ 2`, and `7 ∤ B`)
and pure Darmon–Merel signature `(4,4,13)`.

`hGen_gap14` / `hLLL_gap14` stay `def Prop`. Every `C1 ≥ 1`
loses to `B⁴/A⁴` on `B ≥ B0`; no scaling yields a
Baker–Davenport witness above `17 + C/B0`. Named theorem
`baker_bound_gap14` stays a `def Prop`. 0 sorry. No new axiom.
...
def is_gap14_sol (A B : ℕ) : Prop :=
  A ^ 4 + B ^ 4 = (B + 14) ^ 13

/-- Kernel Baker bound for gap-14. Stays a `def Prop`. -/
def baker_bound_gap14 : Prop :=
  ∀ A B : ℕ, is_gap14_sol A B → B ≤ B0_nat
...
theorem not_gap14_one_zero : ¬ is_gap14_sol 1 0 := by
  unfold is_gap14_sol
  decide

theorem even_fourteen : Even (14 : ℕ) := ⟨7, rfl⟩

theorem prime_seven : Nat.Prime 7 := BealMatveevBeal.BealGap7.prime_seven

/-! ## `gcd(B, B+14) = gcd(B, 14)` divides `14`; odd `B` forces `| 7` -/

theorem gcd_B_C_eq_gcd_fourteen (B : ℕ) :
    Nat.gcd B (B + 14) = Nat.gcd B 14 := by
  rw [Nat.gcd_comm B (B + 14), Nat.add_comm B 14]
  exact (Nat.gcd_add_self_left (14 : ℕ) B).trans (Nat.gcd_comm 14 B)

theorem gcd_B_C_dvd_fourteen (B : ℕ) : Nat.gcd B (B + 14) ∣ 14 := by
  rw [gcd_B_C_eq_gcd_fourteen]
  exact Nat.gcd_dvd_right B 14

theorem gcd_B_C_dvd_fourteen_gap14 (B : ℕ) : Nat.gcd B (B + 14) ∣ 14 :=
  gcd_B_C_dvd_fourteen B
...
theorem no_sol_when_even_B {A B : ℕ}
  ...
  · have hB3o : Odd B3 := Nat.not_even_iff_odd.1 hB3e
    obtain ⟨k, hk⟩ := hB3o
    have hC : 4 * B3 + 7 = 8 * k + 11 := by
      rw [hk]
      ring
    have hC8 : (4 * B3 + 7) % 8 = 3 := by
      rw [hC, Nat.add_mod, Nat.mul_mod]
      norm_num
    have hR : (2 * (4 * B3 + 7) ^ 13) % 16 = 6 :=
      BealMatveevBeal.BealGap6.two_mul_pow13_mod16_of_mod8_three hC8
    obtain h0 | h1 | h2 := BealMatveevBeal.BealGap2.sum_fourth_mod16 A3 B3
    · rw [h0, hR] at hmod
...
theorem B_odd_of_sol_gap14 {A B : ℕ} (h : is_gap14_sol A B) : Odd B := by
...
theorem two_mul_B_pow_four_lt_add_fourteen_pow (B : ℕ) :
    2 * B ^ 4 < (B + 14) ^ 13 := by
  have h14 : 14 ≤ B + 14 := Nat.le_add_left 14 B
  have h2 : 2 ≤ B + 14 := le_trans (by decide : 2 ≤ 14) h14
  have h2pow : 2 ≤ 2 ^ 9 := by decide
  have h29 : 2 ^ 9 ≤ (B + 14) ^ 9 := Nat.pow_le_pow_left h2 9
  have h2le : 2 ≤ (B + 14) ^ 9 := le_trans h2pow h29
  have hBlt : B < B + 14 := Nat.lt_add_of_pos_right (by decide : 0 < (14 : ℕ))
  have hB4 : B ^ 4 < (B + 14) ^ 4 :=
    Nat.pow_lt_pow_left hBlt (by decide : 4 ≠ 0)
  have hle : 2 * B ^ 4 ≤ (B + 14) ^ 9 * B ^ 4 :=
    Nat.mul_le_mul_right (B ^ 4) h2le
  have hposC : 0 < B + 14 := Nat.add_pos_right B (by decide : 0 < 14)
  have hpos9 : 0 < (B + 14) ^ 9 := Nat.pos_pow_of_pos 9 hposC
  have hlt : (B + 14) ^ 9 * B ^ 4 < (B + 14) ^ 9 * (B + 14) ^ 4 :=
    Nat.mul_lt_mul_of_pos_left hB4 hpos9
  have hsum : (B + 14) ^ 9 * (B + 14) ^ 4 = (B + 14) ^ 13 := by
    rw [← pow_add]
  rw [← hsum]
  exact lt_of_le_of_lt hle hlt

theorem A_gt_B_of_sol {A B : ℕ}
    (h : is_gap14_sol A B) (_hB : 0 < B) : B < A := by
  by_contra hle
  ...
  have hlt := two_mul_B_pow_four_lt_add_fourteen_pow B
  have : A ^ 4 + B ^ 4 < (B + 14) ^ 13 := lt_of_le_of_lt hsum hlt
  exact (ne_of_lt this) h

theorem A_ne_zero_of_sol {A B : ℕ} (h : is_gap14_sol A B) : A ≠ 0 := by
  intro hA
  subst hA
  have heq : B ^ 4 = (B + 14) ^ 13 := by
    simpa [is_gap14_sol] using h
  by_cases hB : B = 0
  · subst hB
    exact (by decide : (0 : ℕ) ≠ 793714773254144) heq
...
/-- `C = B+14 ≡ 2 mod 3`, so `C¹³ ≡ 2`, while `A⁴ + B⁴ ≡ A⁴ ∈ {0,1}`.
    Not a 3-descent (`14 ≢ 0 mod 3`). -/
theorem no_sol_when_three_dvd_B_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (h3 : 3 ∣ B) : False := by
  have hB4 : B ^ 4 % 3 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h3 (by decide))
  have hC : (B + 14) % 3 = 2 := by
    have : B % 3 = 0 := Nat.mod_eq_zero_of_dvd h3
    rw [Nat.add_mod, this]
  have hC13 : (B + 14) ^ 13 % 3 = 2 := by
    rw [pow13_mod3, hC]
  have hsum : (A ^ 4 + B ^ 4) % 3 = (B + 14) ^ 13 % 3 := by rw [h]
...
theorem three_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap14_sol A B) : ¬ 3 ∣ B :=
  fun h3 => no_sol_when_three_dvd_B_gap14 h h3

/-! ## `mod 5`: `5 ∣ B` kills residue `C ≡ 4` -/
...
/-- `C = B+14 ≡ 4 mod 5`, so `C¹³ ≡ 4`, while `A⁴ + B⁴ ≡ A⁴ ∈ {0,1}`.
    Not a 5-descent (`14 ≢ 0 mod 5`). -/
theorem no_sol_when_five_dvd_B_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (h5 : 5 ∣ B) : False := by
  have hB4 : B ^ 4 % 5 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h5 (by decide))
  have hC : (B + 14) % 5 = 4 := by
    have : B % 5 = 0 := Nat.mod_eq_zero_of_dvd h5
    rw [Nat.add_mod, this]
  have hC13 : (B + 14) ^ 13 % 5 = 4 := by
    rw [pow13_mod5, hC]
  have hsum : (A ^ 4 + B ^ 4) % 5 = (B + 14) ^ 13 % 5 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
...
theorem five_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap14_sol A B) : ¬ 5 ∣ B :=
  fun h5 => no_sol_when_five_dvd_B_gap14 h h5

/-! ## `mod 7`: fourth powers `{0,1,2,4}`; `7 ∣ B` is a 7-descent -/
...
theorem seven_dvd_of_fourth_pow_mod_zero {n : ℕ}
    (h : n ^ 4 % 7 = 0) : 7 ∣ n :=
  BealMatveevBeal.BealGap7.seven_dvd_of_fourth_pow_mod_zero h
...
/-- `C = B+14 ≡ 0 mod 7`, so `C¹³ ≡ 0` and `A⁴ ≡ 0`, hence `7 ∣ A`. -/
theorem seven_dvd_B_forces_seven_dvd_A_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (h7 : 7 ∣ B) : 7 ∣ A := by
  have hB4 : B ^ 4 % 7 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h7 (by decide))
  have hC : (B + 14) % 7 = 0 := by
    have : B % 7 = 0 := Nat.mod_eq_zero_of_dvd h7
    rw [Nat.add_mod, this]
  have hC13 : (B + 14) ^ 13 % 7 = 0 := by
    rw [pow13_mod7, hC]
  have hsum : (A ^ 4 + B ^ 4) % 7 = (B + 14) ^ 13 % 7 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
  exact seven_dvd_of_fourth_pow_mod_zero hsum

/-- `7 ∣ B` is impossible: three extra divisions by `7` leave
    `343 (A₄⁴ + B₄⁴) = (B₁+2)¹³` with `B₁+2 ≡ 2 mod 7`. -/
theorem no_sol_when_seven_dvd_B_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (h7 : 7 ∣ B) : False := by
  have hA7 := seven_dvd_B_forces_seven_dvd_A_gap14 h h7
  obtain ⟨A1, hA⟩ := hA7
  obtain ⟨B1, hB⟩ := h7
  have hC : B + 14 = 7 * (B1 + 2) := by
    rw [hB, mul_add]
  have hsol1 : (7 * A1) ^ 4 + (7 * B1) ^ 4 = (7 * (B1 + 2)) ^ 13 := by
    rw [← hA, ← hB, ← hC]; exact h
  have heq1 : 2401 * (A1 ^ 4 + B1 ^ 4) = 7 ^ 13 * (B1 + 2) ^ 13 := by
    rw [← sum_fourth_of_mul_seven, hsol1, mul_pow]
  have h9 : A1 ^ 4 + B1 ^ 4 = 7 ^ 9 * (B1 + 2) ^ 13 := by
  ...
  have h5eq : A2 ^ 4 + B2 ^ 4 = 7 ^ 5 * (B1 + 2) ^ 13 := by
  ...
  have h1eq : A3 ^ 4 + B3 ^ 4 = 7 * (B1 + 2) ^ 13 := by
    have hpow : (7 : ℕ) ^ 5 = 2401 * 7 := by norm_num
    apply Nat.eq_of_mul_eq_mul_left (by decide : 0 < 2401)
    rw [heq3, hpow, mul_assoc]
  have hmod3 : (A3 ^ 4 + B3 ^ 4) % 7 = 0 := by
    have : 7 ∣ 7 * (B1 + 2) ^ 13 := dvd_mul_right _ _
  ...
  have heq4src : (7 * A4) ^ 4 + (7 * B4) ^ 4 =
    ...
    rw [← hA4, ← hB4]
    exact h1eq
  have heq4 : 2401 * (A4 ^ 4 + B4 ^ 4) = 7 * (B1 + 2) ^ 13 := by
    rw [← sum_fourth_of_mul_seven]; exact heq4src
  have h343 : 343 * (A4 ^ 4 + B4 ^ 4) = (B1 + 2) ^ 13 := by
  ...
  have hC13 : 7 ∣ (B1 + 2) ^ 13 :=
    ⟨49 * (A4 ^ 4 + B4 ^ 4), by
      rw [← h343, ← mul_assoc]
      rfl⟩
  have hC1 : 7 ∣ B1 + 2 := Nat.Prime.dvd_of_dvd_pow prime_seven hC13
  have hB1seven : 7 ∣ B1 := ⟨B2, hB2⟩
  have hCmod : (B1 + 2) % 7 = 2 := by
    have : B1 % 7 = 0 := Nat.mod_eq_zero_of_dvd hB1seven
    rw [Nat.add_mod, this]
  have hC0 : (B1 + 2) % 7 = 0 := Nat.mod_eq_zero_of_dvd hC1
  rw [hCmod] at hC0
  exact (by decide : (2 : ℕ) ≠ 0) hC0

theorem seven_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap14_sol A B) : ¬ 7 ∣ B :=
  fun h7 => no_sol_when_seven_dvd_B_gap14 h h7

/-! ## Coprimeness: `gcd(A,B) = 1` on every solution -/

theorem gcd_A_B_eq_one_of_sol {A B : ℕ}
    (h : is_gap14_sol A B) : Nat.gcd A B = 1 := by
  have hBodd := B_odd_of_sol_gap14 h
  have h7n := seven_not_dvd_B_of_sol h
  set d := Nat.gcd A B
  have hdA : d ∣ A := Nat.gcd_dvd_left A B
  have hdB : d ∣ B := Nat.gcd_dvd_right A B
  have hdC13 : d ∣ (B + 14) ^ 13 := by
    have hA4 : d ∣ A ^ 4 := dvd_pow hdA (by decide)
    have hB4 : d ∣ B ^ 4 := dvd_pow hdB (by decide)
    have hsum : d ∣ A ^ 4 + B ^ 4 := Nat.dvd_add hA4 hB4
    simpa [is_gap14_sol] using (h ▸ hsum)
  have hgcd14 : Nat.gcd d (B + 14) ∣ 14 := by
    have h1 : Nat.gcd d (B + 14) ∣ B :=
      dvd_trans (Nat.gcd_dvd_left d (B + 14)) hdB
    have h2 : Nat.gcd d (B + 14) ∣ (B + 14) := Nat.gcd_dvd_right d (B + 14)
    exact dvd_trans (Nat.dvd_gcd h1 h2) (gcd_B_C_dvd_fourteen B)
  have hgcd1 : Nat.gcd d (B + 14) = 1 := by
    by_contra hne
    have hpos : 1 < Nat.gcd d (B + 14) :=
      Nat.lt_of_le_of_ne (Nat.succ_le_of_lt
        (Nat.gcd_pos_of_pos_right d (Nat.add_pos_right B (by decide : 0 < 14))))
        (Ne.symm hne)
    obtain ⟨p, hp, hpd⟩ := Nat.exists_prime_and_dvd (ne_of_gt hpos)
    have hp14 : p ∣ 14 := dvd_trans hpd hgcd14
    have hp27 : p = 2 ∨ p = 7 := by
      have hmul : p ∣ 2 ∨ p ∣ 7 :=
        (Nat.Prime.dvd_mul hp).mp (by simpa using hp14)
      rcases hmul with h2 | h7p
      · exact Or.inl
          (((Nat.dvd_prime Nat.prime_two).1 h2).resolve_left hp.ne_one)
      · exact Or.inr
          (((Nat.dvd_prime prime_seven).1 h7p).resolve_left hp.ne_one)
    rcases hp27 with hp2 | hp7
    · have hd2 : 2 ∣ d := by
        have := Nat.gcd_dvd_left d (B + 14)
        rw [hp2] at hpd
    ...
    · have hd7 : 7 ∣ d := by
        ...
        rw [hp7] at hpd
        exact dvd_trans hpd this
      exact h7n (dvd_trans hd7 hdB)
  have hcop : Nat.Coprime d (B + 14) := hgcd1
  have hcop13 : Nat.Coprime d ((B + 14) ^ 13) := hcop.pow_right 13
  have hd1 : d ∣ 1 := by
    have := Nat.dvd_gcd (dvd_refl d) hdC13
    rwa [show Nat.gcd d ((B + 14) ^ 13) = 1 from hcop13] at this
  exact Nat.eq_one_of_dvd_one hd1

theorem gcd_A_B_eq_one_of_sol_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) : Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

theorem coprime_of_sol_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) : Nat.Coprime A B :=
  gcd_A_B_eq_one_of_sol h

theorem gcd_A_B_eq_one_of_remaining_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (_h3 : ¬ 3 ∣ B) (_h5 : ¬ 5 ∣ B) (_h7 : ¬ 7 ∣ B) :
    Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

theorem coprime_of_remaining_gap14 {A B : ℕ}
    (h : is_gap14_sol A B) (h3 : ¬ 3 ∣ B) (h5 : ¬ 5 ∣ B) (h7 : ¬ 7 ∣ B) :
    Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_remaining_gap14 h h3 h5 h7

/-! ## Linear form `Λ = 4 log A − 13 log(B+14)` -/

noncomputable def Lambda_gap14 (A B : ℕ) : ℝ :=
  4 * log (A : ℝ) - 13 * log ((B + 14 : ℕ) : ℝ)

theorem A_pow_four_eq_sub {A B : ℕ} (h : is_gap14_sol A B) :
    A ^ 4 = (B + 14) ^ 13 - B ^ 4 := by
  rw [← h, add_comm, Nat.add_sub_cancel_left]

theorem A_pow_real_add {A B : ℕ} (h : is_gap14_sol A B) :
    (A : ℝ) ^ 4 + (B : ℝ) ^ 4 = ((B + 14 : ℕ) : ℝ) ^ 13 := by
  have h' : ((A ^ 4 + B ^ 4 : ℕ) : ℝ) = (((B + 14) ^ 13 : ℕ) : ℝ) :=
    congrArg (fun n : ℕ => (n : ℝ)) h
  simpa [Nat.cast_add, Nat.cast_pow] using h'

theorem A_pow_pos_of_sol {A B : ℕ} (h : is_gap14_sol A B) :
    (0 : ℝ) < (A : ℝ) ^ 4 := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  exact pow_pos hApos 4

theorem Lambda_gap14_eq_log_div {A B : ℕ}
    (h : is_gap14_sol A B) :
    Lambda_gap14 A B =
      log ((A : ℝ) ^ 4 / ((B + 14 : ℕ) : ℝ) ^ 13) := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  have hCpos : (0 : ℝ) < ((B + 14 : ℕ) : ℝ) := by
    have : 0 < B + 14 := Nat.add_pos_right B (by decide : 0 < 14)
    exact_mod_cast this
  unfold Lambda_gap14
  have h4 : (4 : ℝ) * log (A : ℝ) = log ((A : ℝ) ^ 4) :=
    (Real.log_pow (A : ℝ) 4).symm
  have h13 : (13 : ℝ) * log ((B + 14 : ℕ) : ℝ) =
      log (((B + 14 : ℕ) : ℝ) ^ 13) :=
    (Real.log_pow ((B + 14 : ℕ) : ℝ) 13).symm
  rw [h4, h13, Real.log_div (pow_pos hApos 4).ne' (pow_pos hCpos 13).ne']

theorem abs_Lambda_gap14_eq_log_one_plus {A B : ℕ}
    (h : is_gap14_sol A B) (hB : 0 < B) :
    |Lambda_gap14 A B| = log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) := by
  ...
  have hΛ := Lambda_gap14_eq_log_div h
...
theorem abs_Lambda_gap14_le_ratio {A B : ℕ}
    (h : is_gap14_sol A B) (hB : 0 < B) :
    |Lambda_gap14 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  have hA4pos := A_pow_pos_of_sol h
  have hBpos : (0 : ℝ) < (B : ℝ) := by exact_mod_cast hB
  have hu : (0 : ℝ) < (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
    div_pos (pow_pos hBpos 4) hA4pos
  ...
  have : log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ≤
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
    simpa using hlog
  rwa [abs_Lambda_gap14_eq_log_one_plus h hB]

theorem B_pow_five_add_B_pow_four_lt_add_fourteen_pow {B : ℕ} (_hB : 0 < B) :
    (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 14 : ℕ) : ℝ) ^ 13 := by
  have hx : (0 : ℝ) ≤ (B : ℝ) := Nat.cast_nonneg B
  have hC : ((B + 14 : ℕ) : ℝ) = (B : ℝ) + 14 := by
    rw [Nat.cast_add]
    norm_num
  have h5 : (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 14 : ℕ) : ℝ) ^ 5 := by
    rw [hC]
    have hexp : ((B : ℝ) + 14) ^ 5 =
        (B : ℝ) ^ 5 + 70 * (B : ℝ) ^ 4 + 1960 * (B : ℝ) ^ 3 +
          27440 * (B : ℝ) ^ 2 + 192080 * (B : ℝ) + 537824 := by
      ring
    have h3 : (0 : ℝ) ≤ (B : ℝ) ^ 3 := pow_nonneg hx 3
    have h2 : (0 : ℝ) ≤ (B : ℝ) ^ 2 := sq_nonneg _
    nlinarith [hexp, h3, h2, hx]
  have hC1 : (1 : ℝ) ≤ ((B + 14 : ℕ) : ℝ) := by
    have : 1 ≤ B + 14 := le_trans (by decide : 1 ≤ 14) (Nat.le_add_left 14 B)
    exact_mod_cast this
  have h513 : ((B + 14 : ℕ) : ℝ) ^ 5 ≤ ((B + 14 : ℕ) : ℝ) ^ 13 :=
...
theorem A_pow_gt_B_pow_five {A B : ℕ}
    ...
    (B : ℝ) ^ 5 < (A : ℝ) ^ 4 := by
  have hA4 : (A : ℝ) ^ 4 =
      ((B + 14 : ℕ) : ℝ) ^ 13 - (B : ℝ) ^ 4 := by
    linarith [A_pow_real_add h]
  have hsum := B_pow_five_add_B_pow_four_lt_add_fourteen_pow hB
  linarith [hA4, hsum]
...
theorem abs_Lambda_gap14_lt_inv_B {A B : ℕ}
    (h : is_gap14_sol A B) (hB : 0 < B) :
    |Lambda_gap14 A B| < 1 / (B : ℝ) :=
  lt_of_le_of_lt (abs_Lambda_gap14_le_ratio h hB) (ratio_lt_inv_B h hB)

theorem A_pow_lt_C_pow {A B : ℕ}
    (h : is_gap14_sol A B) (hB : 0 < B) :
    (A : ℝ) ^ 4 < ((B + 14 : ℕ) : ℝ) ^ 13 := by
  have hB4 : 0 < B ^ 4 := Nat.pos_pow_of_pos 4 hB
  have hlt : A ^ 4 < (B + 14) ^ 13 :=
    calc
...
theorem logA_gt_thirteen_of_B_ge_B0 {A B : ℕ}
    (h : is_gap14_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (13 : ℝ) < log (A : ℝ) := by
...
/-- `exp(−C1 log A log(B+14)) < B⁴/A⁴` for every `C1 ≥ 1` on
    a gap-14 solution with `B ≥ B0`. Same cutoff as `62383af`:
    `C1 < 1` would be needed. -/
theorem hGen_exp_lt_ratio_gap14
    {C1 : ℝ} (hC1 : 1 ≤ C1)
    {A B : ℕ} (hsol : is_gap14_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-C1 * log (A : ℝ) * log ((B + 14 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  set a : ℝ := log (A : ℝ)
  set c : ℝ := log ((B + 14 : ℕ) : ℝ)
  set C : ℝ := ((B + 14 : ℕ) : ℝ)
  have hAgt1 : 1 < A := A_gt_one_of_sol_pos hsol hB
  have hCgt1 : 1 < B + 14 :=
    lt_of_lt_of_le (by decide : 1 < 14) (Nat.le_add_left 14 B)
  have hApos : (0 : ℝ) < (A : ℝ) := by
    have : (0 : ℕ) < A := lt_trans Nat.zero_lt_one hAgt1
    exact_mod_cast this
  have hCpos : (0 : ℝ) < C := by
    have : 0 < B + 14 := Nat.add_pos_right B (by decide : 0 < 14)
    have : (0 : ℝ) < ((B + 14 : ℕ) : ℝ) := by exact_mod_cast this
    simpa [C] using this
  have hCgt : (1 : ℝ) < C := by
    have : (1 : ℝ) < ((B + 14 : ℕ) : ℝ) := by exact_mod_cast hCgt1
    simpa [C] using this
...
theorem C1_floor_hGen_loses_gap14 {A B : ℕ}
    (hsol : is_gap14_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-(C1_floor : ℝ) * log (A : ℝ) * log ((B + 14 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
  hGen_exp_lt_ratio_gap14
    (le_of_lt (by
      simpa [C1_floor_eq_logs] using
        BealMatveevBeal.MatveevThreeLogs.C1_floor_gt_one))
    hsol hB hB0

/-! ## Floor form / LLL no-go (`B+14`) -/

noncomputable def floorFormThird_gap14 (C : ℝ) (A B : ℕ) : ℝ :=
  (4 : ℝ) * ⌊C * log (A : ℝ)⌋ - 13 * ⌊C * log ((B + 14 : ℕ) : ℝ)⌋

/-- `|4⌊C log A⌋ − 13⌊C log(B+14)⌋ − C Λ| < 17` for every real `C`. -/
theorem floor_form_approx_of_C_gap14 (C : ℝ) (A B : ℕ) :
    |floorFormThird_gap14 C A B - C * Lambda_gap14 A B| < 17 := by
  set a : ℝ := C * log (A : ℝ)
  set cval : ℝ := C * log ((B + 14 : ℕ) : ℝ)
  have hΛ : C * Lambda_gap14 A B = 4 * a - 13 * cval := by
    simp only [Lambda_gap14, a, cval]
    ring
  have hft : floorFormThird_gap14 C A B = (4 : ℝ) * ⌊a⌋ - 13 * ⌊cval⌋ := rfl
  have hform :
      floorFormThird_gap14 C A B - C * Lambda_gap14 A B =
...
theorem floor_form_third_abs_lt_gap14 {C : ℝ} (hC : 0 < C) (A B : ℕ) :
    |floorFormThird_gap14 C A B| < 17 + C * |Lambda_gap14 A B| := by
  have happ := floor_form_approx_of_C_gap14 C A B
  have hmul : |C * Lambda_gap14 A B| = C * |Lambda_gap14 A B| := by
...
/-- No scaling `C > 0` yields a Baker–Davenport witness
    `r > 17 + C/B0` on a gap-14 solution with `B ≥ B0`. -/
theorem hLLL_method_fails_gap14 :
    ∀ C : ℝ, 0 < C →
      ∀ A B : ℕ,
        is_gap14_sol A B →
          0 < B →
            B0_nat ≤ B →
              ¬ ∃ r : ℝ,
                17 + C / (B0_nat : ℝ) < r ∧
                  r ≤ |floorFormThird_gap14 C A B| := by
  intro C hC A B hsol hB hB0
  rintro ⟨r, hgt, hle⟩
  have htri := floor_form_third_abs_lt_gap14 hC A B
  have hLam := abs_Lambda_gap14_lt_inv_B hsol hB
  have hB0pos : (0 : ℝ) < (B0_nat : ℝ) := by
    have : (0 : ℕ) < B0_nat := by decide
    exact_mod_cast this
  have hBle : (B0_nat : ℝ) ≤ (B : ℝ) := Nat.cast_le.mpr hB0
  have hCΛ : C * |Lambda_gap14 A B| < C / (B0_nat : ℝ) := by
    have hmid : C * |Lambda_gap14 A B| < C * (1 / (B : ℝ)) :=
  ...
  have hthird : |floorFormThird_gap14 C A B| < 17 + C / (B0_nat : ℝ) := by
    linarith [htri, hCΛ]
  linarith [hgt, hle, hthird]

theorem no_uniform_c_cubic_in_N_gap14 :
    ¬ ∃ c : ℝ, 0 < c ∧
      ∀ L N1 N2 N3 : ℕ,
...
theorem coeffCount3_zero_ten_ten_ten_gap14 :
...
theorem no_uniform_c_easy_half_gap14 :
    (1 / 2 : ℝ) * 1 * 1 < 242 := by
  norm_num
...
/-- Matveev 2000 Theorem 1.4, n=2, displayed `C1_floor` exponential
    on a gap-14 solution. Not in Mathlib 4.12. Instantiating it at
...
/-- Bugeaud / LLL close for gap-14: ratio upper bound ⇒ `B ≤ B0`.
    The displayed lattice does not inhabit it. -/
def hLLL_gap14 : Prop :=
  (∀ A B : ℕ, is_gap14_sol A B → 0 < B →
      |Lambda_gap14 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4) →
    baker_bound_gap14

def baker_bound_gap14_of_hGen_hLLL : Prop :=
  hGen_gap14 → hLLL_gap14 → baker_bound_gap14

def baker_bound_gap14_of_hGen_hLLL_gap14 : Prop :=
  baker_bound_gap14_of_hGen_hLLL

/-! ## Combined unconditional no-go (no Darmon–Merel axiom) -/

/-- Log and LLL methods lose on `B ≥ B0`. Remaining solutions are
    coprime with `A > B0`, `B` odd, `A` even. `3 ∣ B`, `5 ∣ B`,
    and `7 ∣ B` are impossible. Darmon–Merel `(4,4,13)` would be
    refuted by any such solution. `baker_bound_gap14` stays
    uninhabited as a named theorem. -/
theorem baker_bound_gap14_unconditional_nogo {A B : ℕ}
    (hsol : is_gap14_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (∀ C1 : ℝ, 1 ≤ C1 →
      exp (-C1 * log (A : ℝ) * log ((B + 14 : ℕ) : ℝ)) <
        (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ∧
  ...
  refine ⟨?_, ?_, coprime_of_sol_gap14 hsol, ?_,
    B_odd_of_sol_gap14 hsol, A_even_of_sol_gap14 hsol,
    three_not_dvd_B_of_sol hsol, five_not_dvd_B_of_sol hsol,
    seven_not_dvd_B_of_sol hsol, ?_⟩
  · intro C1 hC1
    exact hGen_exp_lt_ratio_gap14 hC1 hsol hB hB0
  · intro C hC
    exact hLLL_method_fails_gap14 C hC A B hsol hB hB0
  · exact Nat.lt_of_le_of_lt hB0 (A_gt_B_of_sol hsol hB)
  · intro hDM
    have hA0 : A = 0 ∨ B = 0 :=
      hDM A B (B + 14) (coprime_of_sol_gap14 hsol) hsol
    cases hA0 with
...
/-- Every gap-14 solution is impossible relative to
    `darmon_merel_4413_axiom`. Unlike gap-1 there is no `(1,0)`.
    `B = 0` dies as even. -/
theorem no_gap14_of_axiom {A B : ℕ}
    (hsol : is_gap14_sol A B) : False := by
...
/-- Vacuous `B ≤ B0` for every gap-14 solution, relative to the
    existing Darmon–Merel axiom. Not named `baker_bound_gap14`. -/
theorem baker_bound_gap14_true {A B : ℕ}
    (hsol : is_gap14_sol A B) : B ≤ B0_nat :=
  (no_gap14_of_axiom hsol).elim

theorem baker_bound_gap14_true_of_axiom {A B : ℕ}
    (hsol : is_gap14_sol A B) : B ≤ B0_nat :=
  baker_bound_gap14_true hsol

#check C1_floor_eq
#check B0_nat_eq
#check not_gap14_one_zero
#check gcd_B_C_eq_gcd_fourteen
#check gcd_B_C_dvd_fourteen
#check gcd_B_C_dvd_fourteen_gap14
#check gcd_B_C_dvd_seven_of_odd_gap14
#check gcd_A_B_eq_one_of_sol
#check coprime_of_sol_gap14
#check B_odd_of_sol_gap14
#check A_even_of_sol_gap14
#check no_sol_when_even_B
#check A_gt_B_of_sol
#check no_sol_when_three_dvd_B_gap14
#check no_sol_when_five_dvd_B_gap14
#check seven_dvd_B_forces_seven_dvd_A_gap14
#check no_sol_when_seven_dvd_B_gap14
#check coprime_of_remaining_gap14
#check hGen_exp_lt_ratio_gap14
#check C1_floor_hGen_loses_gap14
#check floor_form_approx_of_C_gap14
#check hLLL_method_fails_gap14
#check no_uniform_c_cubic_in_N_gap14
#check no_uniform_c_easy_half_gap14
#check hGen_gap14
#check hLLL_gap14
#check baker_bound_gap14_of_hGen_hLLL
#check baker_bound_gap14_unconditional_nogo
#check baker_bound_gap14
#check no_gap14_of_axiom
#check baker_bound_gap14_true
#check baker_bound_gap14_true_of_axiom
#print axioms gcd_A_B_eq_one_of_sol
#print axioms B_odd_of_sol_gap14
#print axioms A_even_of_sol_gap14
#print axioms no_sol_when_even_B
#print axioms no_sol_when_three_dvd_B_gap14
#print axioms no_sol_when_five_dvd_B_gap14
#print axioms seven_dvd_B_forces_seven_dvd_A_gap14
#print axioms no_sol_when_seven_dvd_B_gap14
#print axioms hGen_exp_lt_ratio_gap14
#print axioms C1_floor_hGen_loses_gap14
#print axioms hLLL_method_fails_gap14
#print axioms baker_bound_gap14_unconditional_nogo
#print axioms no_gap14_of_axiom
#print axioms baker_bound_gap14_true

end BealMatveevBeal.BealGap14
