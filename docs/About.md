...
stays `def Prop`. `BealGap2.lean` proves
`baker_bound_gap2_unconditional_nogo` for `A⁴+B⁴=(B+2)¹³`:
`B` odd `A` even, `3 ∣ B` impossible like `f1cb039`, same
log/LLL lose, no `(1,0)`. `baker_bound_gap2` stays `def Prop`.
...
`A⁴+B⁴=(B+6)¹³`: `B` odd `A` even, even `B` dies as `6` or `14`
...
`matveev_log_one_plus_le_self` ($\forall x > -1, \log(1+x) \le x$),
`matveev_gap3_B_pow_div_A_pow_pos` ($0 < B^4/A^4$ on solutions),
`matveev_gap3_abs_lambda_nonneg` ($0 \le |\Lambda|$),
`matveev_gap3_abs_lambda_le_ratio` ($|\Lambda| \le B^4/A^4$),
