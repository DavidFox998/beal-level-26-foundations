...
Current Matveev–Beal: `matveev_gap3_lower` is the B≤10^6 integer-gap close (0 sorry); `MatveevLLL.lean` proves 4/13 is a convergent and integer gap < ratio; `MatveevInterpolation.lean` starts p.5 interpolation, proves the bare-real claim is false, and inhabits Cauchy / polynomial vanishing / Siegel G_a jet vanishing on `𝐆_a`; exponential Wüstholz, unrestricted target, `baker_bound_gap3`, and `bugeaud_LLL_reduction_proof` stay def Prop; not v25; concept 22379293.
