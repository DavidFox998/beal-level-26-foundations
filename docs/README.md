...
Current Matveev–Beal: `matveev_gap3_lower` is the B≤10^6 integer-gap close (0 sorry); `MatveevLLL.lean` proves 4/13 is a convergent, integer gap < ratio, `|Λ|<1/B` without a Baker cutoff, that `1/max(4 log A, 13 log(B+3))` is not a lower bound, rank-3 lattice `det=C`, `‖v‖<32`, `λ₁<32`, Lovász/swap/`lll_svt_bound_of_reduced`, `lll_algorithm_terminates` (reduced Z-basis exists), displayed `lll_svt_bound` / `lll_det_bound` (`‖b₁‖≤2^{(n-1)/2}λ₁` and `‖b₁‖≤2^{(n-1)/4}det^{1/n}` on a reduced generating triple), `baker_davenport_no_cutoff` (`(17,18)` gives `10^{-30}` not B≤10⁶), and `floor_form_approx_of_C` (error `<17` for every scaling `C`); `MatveevBugeaud.lean` proves the displayed-lattice no-go (`(r−17)/C < |Λ|`, reduced `‖b₁‖<64`, `C/B0=10^{24}`) and the uniform-`C` method-failure `baker_davenport_reduction_nogo` / `bugeaud_LLL_method_fails`; `WuestholzSubgroup.lean` proves no uniform `c` for intermediate `T≥cLK` (`(1/2)·1·1<242`; linear `K∼N1+N2` fails) and keeps the uniform subgroup theorem as def Prop; `WuestholzProduct.lean` packages full-order `T<coeffCount ∨ P=0 ∨` dependence (`T<242` at `1 10 10`) and keeps `wuestholz_product_theorem_exp` as def Prop; `MatveevThreeLogs.lean` proves the log-method no-go (`Lambda3=Lambda`; any `C1≥1` has `exp(−C1 log A log(B+3)) < B⁴/A⁴` on `B≥B0`; cubic jet `1331`) and keeps `baker_bound_gap3` as def Prop; `BealKraus.lean` proves the displayed Frey / Kraus valuation no-go (`Δ=16 A⁸ B⁸ (B+3)²⁶`; `13∣26` so `C` lowers, `13∤8` so primes in `A` do not) and keeps Darmon–Merel / Ribet as def Prop; `BealKrausZsig.lean` proves the Gaussian/Thue argument-form no-go (`N(A²+iB²)=(B+3)¹³`; `C1<5/log 13<2`; `C1_floor`/`242`/`2` lose to `B⁴/C¹³` on `B≥B0`) and keeps the thirteenth-power factorization / Zsigmondy / Catalan as def Prop; `BealCatalanZsig.lean` proves the Catalan / Zsigmondy gcd-and-shape no-go (`C¹³−B⁴≠1`; `3∤B` ⇒ `gcd(A,B)=1`; Zsigmondy primes in `B` do not force `13∣v_p(A)`; still `C1<2`) and keeps Mihăilescu / Zsigmondy as def Prop; `BealDarmonMerel.lean` proves the Darmon–Merel / Ribet residual-level no-go (`3∣B` ⇒ `3∣A` so gcd≠1; `13∣26` / `13∤8`; unlowered `A`-prime ⇒ residual estimate ∤ 32) and keeps Darmon–Merel / Ribet as def Prop; `BealDarmonMerel44p.lean` proves the (4,4,p) / Ribet-to-32 no-go (displayed residual ≠ 32 on `B>0`; thirteenth powers are Fermat exponent 13) and keeps FLT / Ribet-to-32 as def Prop; `BealFLT13.lean` proves the 3-adic descent no-go (`3∣B` impossible; every solution coprime; remaining case is Darmon–Merel / FLT 13, not in Mathlib 4.12) and keeps those as def Prop; `BealGenuineV25.lean` proves Darmon–Merel `(4,4,13)` would close `baker_bound_gap3` vacuously (not a theorem here; a `B≥B0` solution refutes it) and keeps DM / FLT 13 / Ribet-to-32 as def Prop; `BealTrueV25.lean` inhabits `baker_bound_gap3_true` relative to `axiom darmon_merel_4413_axiom` (0 sorry; extra axiom; not Mathlib 4.12; not the minted unconditional v25 tag); `BealUnconditionalV25.lean` proves `hGen`+`hLLL` cannot cut `B≤10⁶` (`baker_bound_gap3_unconditional_nogo`; those stay def Prop); `BealGap1.lean` proves `baker_bound_gap1_unconditional_nogo` for `A⁴+B⁴=(B+1)¹³` (coprime from the start; `3∣B` compatible; `(1,0)` is a solution; `baker_bound_gap1` stays def Prop); `BealGap2.lean` proves `baker_bound_gap2_unconditional_nogo` for `A⁴+B⁴=(B+2)¹³` (`B` odd `A` even; `3∣B` impossible like `f1cb039`; no `(1,0)`; `baker_bound_gap2` stays def Prop); `BealGap4.lean` proves `baker_bound_gap4_unconditional_nogo` for `A⁴+B⁴=(B+4)¹³` (`B` odd `A` even; `3∣B` compatible like gap-1; `5∣B` impossible; no `(1,0)`; `baker_bound_gap4` stays def Prop); `BealGap5.lean` proves `baker_bound_gap5_unconditional_nogo` for `A⁴+B⁴=(B+5)¹³` (`B` even `A` odd; `3∣B` impossible like `f1cb039`; `5∣B` impossible by 5-descent; no `(1,0)`; `baker_bound_gap5` stays def Prop); `BealGap6.lean` proves `baker_bound_gap6_unconditional_nogo` for `A⁴+B⁴=(B+6)¹³` (`B` odd `A` even; even `B` dies as `6` or `14` mod 16; `3∣B` impossible by 3-descent `C₁=B₁+2`; `5∣B` compatible; no `(1,0)`; `baker_bound_gap6` stays def Prop); `BealGap7.lean` proves `baker_bound_gap7_unconditional_nogo` for `A⁴+B⁴=(B+7)¹³` (`B` even `A` odd; `3∣B` compatible; `5∣B` impossible `C≡2`; `7∣B` impossible by 7-descent; no `(1,0)`; `baker_bound_gap7` stays def Prop); `MatveevInterpolation.lean` starts p.5 interpolation, proves the bare-real claim is false, and inhabits Cauchy / polynomial vanishing / Siegel G_a jet vanishing / full-order `P=0 ∨` dependence / `𝐆_a × 𝐆_m²` candidate classification (graph ⇒ dependence; independence keeps `W` out); intermediate `T ≥ c L K` is strictly weaker than full jet order and stays def Prop uniformly in `N1,N2`; unrestricted target and `bugeaud_LLL_reduction_proof` stay def Prop; named theorem `baker_bound_gap3` stays uninhabited (`baker_bound_gap3_true` is axiom-relative in `BealTrueV25`); not a minted unconditional v25 tag; concept 22379293.
...
# Beal Level 26 Foundations — Exponent 13 Full Package (Computational Boundary) — v29
...
v8.85.0-paper-B14-full BealFreyModQKill/paper 352-row residue cover beal_..._B14_full (266 mod53 + 77 q=29 + 9 q=109); #print axioms empty; KrausB14 eliminated_full stays 2-element mod16; B14_honest stays uninhabited Prop; eliminated_86 pack stays Prop; Chain ExistsNewformLevel2 stays 0≠0; Density/Step kraus ∀ stays uninhabited; beal_from_ribet still from the Prop. Still not Full Mathlib ∀.
v8.84.0-B14-modq-kill BealFreyModQKill fourth-power non-residue kill of the 86 mod-53 survivors at q=17/29/109; 29 kills 77 and 109 kills the remaining 9; #print axioms empty on direct_modQ_kill; not Ribet; B14_honest / 352-row ∀ stays uninhabited Prop; eliminated_86 pack stays the uninhabited Prop; Chain ExistsNewformLevel2 stays 0≠0; Density/Step kraus ∀ stays uninhabited; beal_from_ribet still from the Prop. Still not Full Mathlib ∀.
v8.83.0-B14-86-props-honest BealFreyB14 gcd(B,B+3)∣3 inhabited; IrreducibleMod13Trace of Euler a53 miss on the 86 (not Mazur); conductor_86 / level_lowering_86 / eliminated_86 stay uninhabited Props; Classical.choice is not BCDT/Ribet; Zsig 5983 is not N|32*3*13; Chain ExistsNewformLevel2 stays 0≠0; Density/Step kraus ∀ stays uninhabited; beal_from_ribet still from the Prop. Still not Full Mathlib ∀.
v8.82.0-B14-a53-miss-86 BealFreyASearch/BealFreyB14 Euler a53 of Beal Frey cubic on the 86 mod-53 survivors is in {-10,-2,1,6,14} and misses 0,12 at ℓ=13 under hEq; [propext, Quot.sound]; not ¬∃ A for those 86; B14_honest stays Prop; not Ribet; Chain ExistsNewformLevel2 stays 0≠0; Density/Step kraus ∀ stays uninhabited; beal_from_ribet still from the Prop. Still not Full Mathlib ∀.
v8.81.0-B14-mod53-kill BealFreyMod53Kill 266 of 352 named B≡14 rows die because (B+3)^13-B^4 is not a fourth power mod 53; #print axioms empty; 86 survive this test; B14_honest stays uninhabited ∀ on all 352; not Ribet; Chain ExistsNewformLevel2 stays 0≠0; Density/Step kraus ∀ stays uninhabited; beal_from_ribet still from the Prop. Still not Full Mathlib ∀.
