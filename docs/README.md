...
Current Matveev–Beal: `matveev_gap3_lower` is the B≤10^6 integer-gap close (0 sorry); `MatveevLLL.lean` proves 4/13 is a convergent and integer gap < ratio; `MatveevInterpolation.lean` starts p.5 interpolation, proves the bare-real claim is false, and inhabits Cauchy / polynomial vanishing / Siegel G_a jet vanishing / full-order `P=0 ∨` dependence / `𝐆_a × 𝐆_m²` candidate classification (graph ⇒ dependence; independence keeps `W` out); intermediate `T ≥ c L K` is strictly weaker than full jet order and stays def Prop uniformly in `N1,N2`; unrestricted target, `baker_bound_gap3`, and `bugeaud_LLL_reduction_proof` stay def Prop; not v25; concept 22379293.
