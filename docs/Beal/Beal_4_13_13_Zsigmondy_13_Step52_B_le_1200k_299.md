# v8.57.0-B-le-1200k-3591-rows Zsigmondy Step 52

`lean/BealLevel26Foundations/Beal/FullProof/Beal_4_13_13_Zsigmondy_13_Step52_B_le_1200k_299.lean`

Extends the B<=1100000 table (3292 named
gap-3 pairs, including the inhabited
outlier (200000, 200003) with Pratt
p=12186951011) to 3591 named rows with
B<=1200000 (C=B+3).  Inherited Step51
rows are wrapped.  The 299 new rows
in (1100000, 1200000] are sampled evenly
from the 50366-row pool of B with a
...
First new row (1100002, 1100005) p=53.
Named last row (1199998, 1200001) p=131.
B=1200000 is not in the pool.
Distribution of the sampled 299:
53:128, 79:72, 131:36, 157:28, 547:11,
443:9, 313:9, 521:6.
...
This is not every `B <= 1200000`.
`exists_p_with_order_ne_13_mod_p_sq_inhabited`
stays a Prop.  `B > 1200000` squarefull
...
| Name | Status |
|---|---|
| `S_has_prime_with_exp_one_B_le_1200000_table_rows` | inhabited (3591 rows) |
| `exists_p_with_order_ne_13_B_le_1200000_from_exp_one_table_rows` | inhabited (3591 rows) |
| `S_not_fourth_B_le_1200000_from_exp_one_table_rows` | inhabited (3591 rows) |
| `row_1100002_1100005` | inhabited (p=53) |
| `row_1199998_1200001` | inhabited (p=131) |
...
| `bugeaud_P_phi13_gt_C_when_B_gt_1200000` | uninhabited |
| `rad_S_gt_sqrt_13_C12_when_squarefull_B_gt_1200000` | uninhabited |
