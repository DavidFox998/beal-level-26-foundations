# v8.60.0-B-le-1500k-4488-rows Zsigmondy Step 55

`lean/BealLevel26Foundations/Beal/FullProof/Beal_4_13_13_Zsigmondy_13_Step55_B_le_1500k_299.lean`

Extends the B<=1400000 table (4189 named
gap-3 pairs, including the inhabited
outlier (200000, 200003) with Pratt
p=12186951011) to 4488 named rows with
B<=1500000 (C=B+3).  Inherited Step54
rows are wrapped.  The 299 new rows
in (1400000, 1500000] are sampled evenly
from the 50335-row pool of B with a
...
First new row (1400001, 1400004) p=53.
Named last row (1499999, 1500002) p=53.
B=1500000 is not in the pool.
Distribution of the sampled 299:
53:154, 79:55, 131:37, 157:22, 313:14,
443:11, 521:4, 547:2.
...
This is not every `B <= 1500000`.
`exists_p_with_order_ne_13_mod_p_sq_inhabited`
stays a Prop.  `B > 1500000` squarefull
...
| Name | Status |
|---|---|
| `S_has_prime_with_exp_one_B_le_1500000_table_rows` | inhabited (4488 rows) |
| `exists_p_with_order_ne_13_B_le_1500000_from_exp_one_table_rows` | inhabited (4488 rows) |
| `S_not_fourth_B_le_1500000_from_exp_one_table_rows` | inhabited (4488 rows) |
| `row_1400001_1400004` | inhabited (p=53) |
| `row_1499999_1500002` | inhabited (p=53) |
...
| `bugeaud_P_phi13_gt_C_when_B_gt_1500000` | uninhabited |
| `rad_S_gt_sqrt_13_C12_when_squarefull_B_gt_1500000` | uninhabited |
