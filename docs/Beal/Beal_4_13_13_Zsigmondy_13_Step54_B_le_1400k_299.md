# v8.59.0-B-le-1400k-4189-rows Zsigmondy Step 54

`lean/BealLevel26Foundations/Beal/FullProof/Beal_4_13_13_Zsigmondy_13_Step54_B_le_1400k_299.lean`

Extends the B<=1300000 table (3890 named
gap-3 pairs, including the inhabited
outlier (200000, 200003) with Pratt
p=12186951011) to 4189 named rows with
B<=1400000 (C=B+3).  Inherited Step53
rows are wrapped.  The 299 new rows
in (1300000, 1400000] are sampled evenly
from the 50314-row pool of B with a
...
First new row (1300002, 1300005) p=53.
Named last row (1400000, 1400003) p=53.
B=1400000 is in the pool.
Distribution of the sampled 299:
53:129, 79:78, 131:41, 157:25, 313:13,
443:7, 521:5, 547:1.
...
This is not every `B <= 1400000`.
`exists_p_with_order_ne_13_mod_p_sq_inhabited`
stays a Prop.  `B > 1400000` squarefull
...
| Name | Status |
|---|---|
| `S_has_prime_with_exp_one_B_le_1400000_table_rows` | inhabited (4189 rows) |
| `exists_p_with_order_ne_13_B_le_1400000_from_exp_one_table_rows` | inhabited (4189 rows) |
| `S_not_fourth_B_le_1400000_from_exp_one_table_rows` | inhabited (4189 rows) |
| `row_1300002_1300005` | inhabited (p=53) |
| `row_1400000_1400003` | inhabited (p=53) |
...
| `bugeaud_P_phi13_gt_C_when_B_gt_1400000` | uninhabited |
| `rad_S_gt_sqrt_13_C12_when_squarefull_B_gt_1400000` | uninhabited |
