...
- [`BealMatveevBealV25B0Search.md`](BealMatveevBealV25B0Search.md) — separate Lake target: mod-16 + floor fourth-root checker; `check_range` is foldl; `B<1000` is ten shards of 100; `|Λ| ≤ 2/B⁹` on `B≥100`; `∀ B ≤ B0` and `|Λ| ≥ B⁻⁸` stay `def Prop`; `LLLTargetB8` lattices `C=10⁴⁸` `L'` and 3-dim `L3` `C₃=10³⁰` `λ₁=1`; need `C1'≤8` vs typical `30`–`50`; implication is not an inhabitant; `Gap3B0Million` wires 10k foldl shards as an implication (`allShardsTrue_eq_true` stays `def Prop`); `LLLTargetB8_C1_lower_bound` proves `C1'≤8` impossible on a solution (`2/B⁹ < 1/B⁸`); `LLL_cannot_reach_B8` stays `def Prop`; `DarmonMerelFrey4413` displayed Frey `(4,4,13)`; `level_32_no_newform` stays `def Prop`; generator refuses `Beal/Matveev/`; v25 not minted
