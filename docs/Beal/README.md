...
- [`BealMatveevBealV25B0Search.md`](BealMatveevBealV25B0Search.md) — separate Lake target: mod-16 + floor fourth-root checker; `check_range` is foldl; `B<1000` is ten shards of 100; `|Λ| ≤ 2/B⁹` on `B≥100`; `∀ B ≤ B0` and `|Λ| ≥ B⁻⁸` stay `def Prop`; `LLLTargetB8` lattice `C=10³⁰` `λ₁≥1` with integer-length nogo; implication is not an inhabitant; v25 not minted
