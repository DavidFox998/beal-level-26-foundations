...
- [`PAdicLLL.md`](PAdicLLL.md) — `PadicInt` alias; `¬ A ≤ B+10` on gap solutions with `B≥2`; floor lattice `b1=(1,0)` has length `1 < B0` (`lll_euclidean_lower_bound_fails`); `p_adic_LLL_reduction` / `LLL_reduces_bound_to_B0` / `LLL_reduces_bound_to_B0_theorem` stay `def Prop`; v25 not minted
