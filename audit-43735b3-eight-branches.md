...
## Source-drift repair and clean rerun on 2026-09-22

The five affected branch lines were advanced without changing their merge base
from `43735b3b14a9e844d4b4bdf57fa5f68d29e749ee`. An aggregate repair branch,
`fix-source-drift-43735b3`, was also constructed from that exact baseline.

The repaired identifiers are:

- `thirteen_dvd_valuation_discriminant_at_29` now applies the proved lower-level
  theorem
  `BealMatveevBeal.Tate_I29_Inertia.thirteen_dvd_padic_valuation_Delta`
  directly rather than resolving `thirteen_dvd_v29_Delta_reexport`;
- `two_pow_sel2_dimensions_one_numerical` now composes
  `two_pow_sel2_F2_dim_26a1` and `two_pow_sel2_F2_dim_26b1` with the already
  proved numerical cardinality theorems
  `sel2_card_26a1_one_numerical` and `sel2_card_26b1_one_numerical`;
- `curve26a1_discriminant_numerical` and
  `curve26b1_discriminant_numerical` now apply the proved lower-level
  discriminant theorems `curve26a1_Δ` and `curve26b1_Δ` directly rather than
  resolving the later `_reexport` aliases.

No proposition-valued placeholder was inhabited or weakened. No `def Prop`,
`sorry`, `native_decide`, or axiom was added. The transparent library-scale
boundaries listed above remain uninhabited. The repaired Tate theorem has only
the standard dependencies `propext`, `Classical.choice`, and `Quot.sound`.
The repaired combined Kolyvagin numerical surface has the same standard
dependency envelope; its Selmer-cardinality theorem itself has no axioms, and
the two-power theorem reports only `propext`.

The earlier failure was a mixed source/artifact state: direct elaboration of a
numerical source file saw a stale project `.olean` for its parent module.
Conversely, a maximal build could remain green when a current target `.olean`
already masked the checked-out source. A reliable source check therefore needs
both halves in order: delete the project `.lake/build`, rebuild the maximal
registered root from the checked-out source, and only then run direct-file
elaboration and the referee. Pinned Mathlib binary artifacts may be restored
between `lake clean` and the project build; they do not substitute for project
source elaboration.

After the repair, each changed branch was independently checked with
`lake clean`, pinned dependency-cache restoration, a maximal registered build,
and the publication validator. All six changed heads passed:

| Checkout | Repaired head | Result |
|---|---|---|
| `fix-source-drift-43735b3` | `89030a4929abb6532d7644b852cb17911421d22b` | PASS |
| `tate-v29-numerical` | `e024456eb84c85f189918430b70807e1d43ba536` | PASS |
| `kolyvagin-fintype-numerical` | `2342be9b0b7c31ec13cf0af5ec23f216d509595a` | PASS |
| `final-bridge-honest-43735b3` | `53eb69a4acfcde634240d7463d9cfbab58fd8f4c` | PASS |
| `tate-lib-scaffold` | `9d3c7fb4c0fa933932a0c5bf2606ac33b8c7f026` | PASS |
| `kolyvagin-lib-scaffold` | `1f277211ce21e697046b81765bd7d470ef5e252b` | PASS |

The complete eleven-head matrix was then rerun against remote heads. Before
every head, the project `.lake/build` directory was deleted; the maximal root
was rebuilt before direct-file validation. Every row passed:

| Checkout | Head | Build jobs | Result |
|---|---|---:|---|
| baseline `43735b3` | `43735b3b14a9e844d4b4bdf57fa5f68d29e749ee` | `5175/5176` | PASS |
| `mazur-x0-13-numerical` | `ad2b13dced0a787ba44ebc0cef3233ebc11a0d2c` | `5176/5177` | PASS |
| `ribet-928-32-numerical` | `cf2b171f4fbbb95f5573b6a8b8d916b9776c55cf` | `5176/5177` | PASS |
| `mazur-lib-scaffold` | `8330a7ca2dc3d3efacb1382b31ec64a4269485e4` | `5180/5181` | PASS |
| `ribet-lib-scaffold` | `801bbecc6af2adf473ea209b015c512b0fea8ba4` | `5183/5184` | PASS |
| `audit-43735b3-eight-branches` | `3f48ba73b4b2eb1b2122a9dfbb2c74e50635fc50` | `5175/5176` | PASS |
| `tate-v29-numerical` | `e024456eb84c85f189918430b70807e1d43ba536` | `5176/5177` | PASS |
| `kolyvagin-fintype-numerical` | `2342be9b0b7c31ec13cf0af5ec23f216d509595a` | `5176/5177` | PASS |
| `final-bridge-honest-43735b3` | `53eb69a4acfcde634240d7463d9cfbab58fd8f4c` | `5180/5181` | PASS |
| `tate-lib-scaffold` | `9d3c7fb4c0fa933932a0c5bf2606ac33b8c7f026` | `5181/5182` | PASS |
| `kolyvagin-lib-scaffold` | `1f277211ce21e697046b81765bd7d470ef5e252b` | `5184/5185` | PASS |

The scaffold and bridge totals cover the expected registered-job range
5180--5185. Every validator run also passed the maximal build, referee, direct
source elaboration, exact merge-base checks, and proof-level `sorry` /
`sorryAx` checks.