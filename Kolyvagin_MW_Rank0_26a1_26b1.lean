...
`BSD_MordellWeil` (`8994d38`) keeps `IsRankZero := Subsingleton`
and **refutes** it on Cremona `26a1` / `26b1`: torsion points
`(4,4)` and `(1,0)` are not `0`, so `¬ IsRankZero`. That pasted
name is only the identity, not MW rank 0.
...
/-- Algebraic MW rank 0 as finiteness of the rational points.
    Not `IsRankZero` (that is Subsingleton / only `{0}`).
    Uninhabited: Mathlib 4.12 has no Mordell–Weil
...
#check not_IsRankZero_26a1_reexport
#check not_IsRankZero_26b1_reexport
