...
That does **not** inhabit unrestricted `matveev_gap3_lower` (the product
is still `O((log(B+3))³)` and does not beat `B⁴/A⁴` for large `B`) and
does **not** inhabit `baker_bound_gap3`. Not v25.
-/
