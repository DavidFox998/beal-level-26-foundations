...
/-!
# Gap-5 `A⁴ + B⁴ = (B+5)¹³` (not a `B ≤ 10⁶` close)

Successor of `5388a60` (`v24-v24x-final-gap4-nogo`).
`C = B+5` so `gcd(B,C) ∣ 5`. Parity is gap-1, not gap-2/4:
...
`A⁴ ∈ {0,1}`. `5 ∣ B` forces `5 ∣ A` and then a 5-descent
dies: three extra divisions leave
`125 (A₄⁴ + B₄⁴) = (125 B₄ + 1)¹³` with the right side
`≡ 1` mod 5. Remaining solutions are coprime (`p ∣ A,B`
⇒ `p ∣ 5`, but `5 ∤ B`) and pure Darmon–Merel `(4,4,13)`.

`hGen_gap5` / `hLLL_gap5` stay `def Prop`. Every `C1 ≥ 1` loses
to `B⁴/A⁴` on `B ≥ B0`; no scaling yields a Baker–Davenport
witness above `17 + C/B0`. Named theorem `baker_bound_gap5`
...
def is_gap5_sol (A B : ℕ) : Prop :=
  A ^ 4 + B ^ 4 = (B + 5) ^ 13

/-- Kernel Baker bound for gap-5. Stays a `def Prop`. -/
def baker_bound_gap5 : Prop :=
  ∀ A B : ℕ, is_gap5_sol A B → B ≤ B0_nat
...
theorem not_gap5_one_zero : ¬ is_gap5_sol 1 0 := by
  unfold is_gap5_sol
  decide

theorem odd_five : Odd (5 : ℕ) := ⟨2, rfl⟩

/-! ## `gcd(B, B+5) = gcd(B, 5)` divides `5` -/

theorem gcd_B_C_eq_gcd_five (B : ℕ) : Nat.gcd B (B + 5) = Nat.gcd B 5 := by
  rw [Nat.gcd_comm B (B + 5), Nat.add_comm B 5]
  exact (Nat.gcd_add_self_left (5 : ℕ) B).trans (Nat.gcd_comm 5 B)

theorem gcd_B_C_dvd_five (B : ℕ) : Nat.gcd B (B + 5) ∣ 5 := by
  rw [gcd_B_C_eq_gcd_five]
  exact Nat.gcd_dvd_right B 5

theorem gcd_B_C_dvd_five_gap5 (B : ℕ) : Nat.gcd B (B + 5) ∣ 5 :=
  gcd_B_C_dvd_five B

/-! ## Parity: `B` is even, `A` is odd (gap-1, opposite gap-2/4) -/

theorem B_even_of_sol_gap5 {A B : ℕ} (h : is_gap5_sol A B) : Even B := by
  by_contra hne
  have hBodd : Odd B := Nat.not_even_iff_odd.1 hne
  have hCeven : Even (B + 5) := hBodd.add_odd odd_five
  have h16C : 16 ∣ (B + 5) ^ 4 :=
    BealCatalanZsig.even_pow_four_dvd_sixteen hCeven
  have h16R : 16 ∣ (B + 5) ^ 13 :=
    dvd_trans h16C (pow_dvd_pow (B + 5) (by decide : 4 ≤ 13))
  have hR : (B + 5) ^ 13 % 16 = 0 := Nat.mod_eq_zero_of_dvd h16R
  have hB4 : B ^ 4 % 16 = 1 :=
  ...
  · have hA4 : A ^ 4 % 16 = 0 :=
    ...
    have : (A ^ 4 + B ^ 4) % 16 = (B + 5) ^ 13 % 16 := by rw [h]
    rw [hL, hR] at this
...
theorem A_odd_of_sol_gap5 {A B : ℕ} (h : is_gap5_sol A B) : Odd A := by
  have hBeven := B_even_of_sol_gap5 h
  have hCodd : Odd (B + 5) := hBeven.add_odd odd_five
  have hC13 : Odd ((B + 5) ^ 13) := hCodd.pow
...
theorem two_mul_B_pow_four_lt_add_five_pow (B : ℕ) :
    2 * B ^ 4 < (B + 5) ^ 13 := by
  have h5 : 5 ≤ B + 5 := Nat.le_add_left 5 B
  have h2 : 2 ≤ B + 5 := le_trans (by decide : 2 ≤ 5) h5
  have h2pow : 2 ≤ 2 ^ 9 := by decide
  have h29 : 2 ^ 9 ≤ (B + 5) ^ 9 := Nat.pow_le_pow_left h2 9
  have h2le : 2 ≤ (B + 5) ^ 9 := le_trans h2pow h29
  have hBlt : B < B + 5 := Nat.lt_add_of_pos_right (by decide : 0 < (5 : ℕ))
  have hB4 : B ^ 4 < (B + 5) ^ 4 :=
    Nat.pow_lt_pow_left hBlt (by decide : 4 ≠ 0)
  have hle : 2 * B ^ 4 ≤ (B + 5) ^ 9 * B ^ 4 :=
    Nat.mul_le_mul_right (B ^ 4) h2le
  have hposC : 0 < B + 5 := Nat.add_pos_right B (by decide : 0 < 5)
  have hpos9 : 0 < (B + 5) ^ 9 := Nat.pos_pow_of_pos 9 hposC
  have hlt : (B + 5) ^ 9 * B ^ 4 < (B + 5) ^ 9 * (B + 5) ^ 4 :=
    Nat.mul_lt_mul_of_pos_left hB4 hpos9
  have hsum : (B + 5) ^ 9 * (B + 5) ^ 4 = (B + 5) ^ 13 := by
    rw [← pow_add]
  rw [← hsum]
  exact lt_of_le_of_lt hle hlt

theorem A_gt_B_of_sol {A B : ℕ}
    (h : is_gap5_sol A B) (_hB : 0 < B) : B < A := by
  by_contra hle
  ...
  have hlt := two_mul_B_pow_four_lt_add_five_pow B
  have : A ^ 4 + B ^ 4 < (B + 5) ^ 13 := lt_of_le_of_lt hsum hlt
  exact (ne_of_lt this) h

theorem A_ne_zero_of_sol {A B : ℕ} (h : is_gap5_sol A B) : A ≠ 0 := by
  intro hA
  subst hA
  have heq : B ^ 4 = (B + 5) ^ 13 := by
    simpa [is_gap5_sol] using h
  by_cases hB : B = 0
  · subst hB
    exact (by decide : (0 : ℕ) ≠ 1220703125) heq
...
/-- `C = B+5 ≡ 2 mod 3`, so `C¹³ ≡ 2`, while `A⁴ + B⁴ ≡ A⁴ ∈ {0,1}`. -/
theorem no_sol_when_three_dvd_B_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) (h3 : 3 ∣ B) : False := by
  have hB4 : B ^ 4 % 3 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h3 (by decide))
  have hC : (B + 5) % 3 = 2 := by
    have : B % 3 = 0 := Nat.mod_eq_zero_of_dvd h3
    rw [Nat.add_mod, this]
  have hC13 : (B + 5) ^ 13 % 3 = 2 := by
    rw [pow13_mod3, hC]
  have hsum : (A ^ 4 + B ^ 4) % 3 = (B + 5) ^ 13 % 3 := by rw [h]
...
theorem three_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap5_sol A B) : ¬ 3 ∣ B :=
  fun h3 => no_sol_when_three_dvd_B_gap5 h h3
...
theorem both_five_dvd_of_sum_fourth_mod5_zero {X Y : ℕ}
    (h : (X ^ 4 + Y ^ 4) % 5 = 0) : 5 ∣ X ∧ 5 ∣ Y := by
  obtain hX0 | hX1 := fourth_pow_mod5 X
  · obtain hY0 | hY1 := fourth_pow_mod5 Y
    · exact ⟨five_dvd_of_fourth_pow_mod_zero hX0,
        five_dvd_of_fourth_pow_mod_zero hY0⟩
    · have hL : (X ^ 4 + Y ^ 4) % 5 = 1 := by
  ...
  · obtain hY0 | hY1 := fourth_pow_mod5 Y
    · have hL : (X ^ 4 + Y ^ 4) % 5 = 1 := by
      ...
      rw [hL] at h
      exact False.elim ((by decide : (1 : ℕ) ≠ 0) h)
    · have hL : (X ^ 4 + Y ^ 4) % 5 = 2 := by
        rw [Nat.add_mod, hX1, hY1]
      rw [hL] at h
      exact False.elim ((by decide : (2 : ℕ) ≠ 0) h)

theorem sum_fourth_of_mul_five (x y : ℕ) :
    (5 * x) ^ 4 + (5 * y) ^ 4 = 625 * (x ^ 4 + y ^ 4) := by
  have hx : (5 * x) ^ 4 = 625 * x ^ 4 := by
    rw [mul_pow]; norm_num
  have hy : (5 * y) ^ 4 = 625 * y ^ 4 := by
    rw [mul_pow]; norm_num
  rw [hx, hy, mul_add]

/-- `C = B+5 ≡ 0 mod 5`, so `C¹³ ≡ 0` and `A⁴ ≡ 0`, hence `5 ∣ A`. -/
theorem five_dvd_B_forces_five_dvd_A_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) (h5 : 5 ∣ B) : 5 ∣ A := by
  have hB4 : B ^ 4 % 5 = 0 :=
    Nat.mod_eq_zero_of_dvd (dvd_pow h5 (by decide))
  have hC : (B + 5) % 5 = 0 := by
    have : B % 5 = 0 := Nat.mod_eq_zero_of_dvd h5
    rw [Nat.add_mod, this]
  have hC13 : (B + 5) ^ 13 % 5 = 0 := by
    rw [pow13_mod5, hC]
  have hsum : (A ^ 4 + B ^ 4) % 5 = (B + 5) ^ 13 % 5 := by rw [h]
  rw [Nat.add_mod, hB4, Nat.add_zero, Nat.mod_mod, hC13] at hsum
...
/-- `5 ∣ B` is impossible: three extra divisions by `5` leave
    `125 (A₄⁴ + B₄⁴) = (125 B₄ + 1)¹³` with the right side
    `≡ 1` mod 5. -/
theorem no_sol_when_five_dvd_B_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) (h5 : 5 ∣ B) : False := by
  have hA5 := five_dvd_B_forces_five_dvd_A_gap5 h h5
  obtain ⟨A1, hA⟩ := hA5
  obtain ⟨B1, hB⟩ := h5
  have hC : B + 5 = 5 * (B1 + 1) := by
    rw [hB, mul_add, mul_one]
  have hsol1 : (5 * A1) ^ 4 + (5 * B1) ^ 4 = (5 * (B1 + 1)) ^ 13 := by
    rw [← hA, ← hB, ← hC]; exact h
  have heq1 : 625 * (A1 ^ 4 + B1 ^ 4) = 5 ^ 13 * (B1 + 1) ^ 13 := by
    rw [← sum_fourth_of_mul_five, hsol1, mul_pow]
  have h9 : A1 ^ 4 + B1 ^ 4 = 5 ^ 9 * (B1 + 1) ^ 13 := by
  ...
  have heq4src : (5 * A4) ^ 4 + (5 * B4) ^ 4 =
      5 * (125 * B4 + 1) ^ 13 := by
    have hB21 : 25 * B3 + 1 = 125 * B4 + 1 := by
      rw [hB4]
      have hmul : 25 * (5 * B4) = 125 * B4 := by
        rw [← mul_assoc]; rfl
      rw [hmul]
    rw [← hA4, ← hB4, ← hB21]; exact h1eq
  have heq4 : 625 * (A4 ^ 4 + B4 ^ 4) = 5 * (125 * B4 + 1) ^ 13 := by
    rw [← sum_fourth_of_mul_five]; exact heq4src
  have hlast : 125 * (A4 ^ 4 + B4 ^ 4) = (125 * B4 + 1) ^ 13 := by
  ...
  have hRmod : (125 * B4 + 1) ^ 13 % 5 = 1 := by
    have hC : (125 * B4 + 1) % 5 = 1 := by
      rw [Nat.add_mod, Nat.mul_mod]
      norm_num
    rw [pow13_mod5, hC]
  have hmod : (125 * (A4 ^ 4 + B4 ^ 4)) % 5 =
      (125 * B4 + 1) ^ 13 % 5 := by rw [hlast]
  rw [hLmod, hRmod] at hmod
  exact (by decide : (0 : ℕ) ≠ 1) hmod

theorem five_not_dvd_B_of_sol {A B : ℕ}
    (h : is_gap5_sol A B) : ¬ 5 ∣ B :=
  fun h5 => no_sol_when_five_dvd_B_gap5 h h5

/-! ## Coprimeness: `gcd(A,B) = 1` on every solution -/

theorem gcd_A_B_eq_one_of_sol {A B : ℕ}
    (h : is_gap5_sol A B) : Nat.gcd A B = 1 := by
  have h5n := five_not_dvd_B_of_sol h
  set d := Nat.gcd A B
  have hdA : d ∣ A := Nat.gcd_dvd_left A B
  have hdB : d ∣ B := Nat.gcd_dvd_right A B
  have hdC13 : d ∣ (B + 5) ^ 13 := by
    have hA4 : d ∣ A ^ 4 := dvd_pow hdA (by decide)
    have hB4 : d ∣ B ^ 4 := dvd_pow hdB (by decide)
    have hsum : d ∣ A ^ 4 + B ^ 4 := Nat.dvd_add hA4 hB4
    simpa [is_gap5_sol] using (h ▸ hsum)
  have hgcd5 : Nat.gcd d (B + 5) ∣ 5 := by
    have h1 : Nat.gcd d (B + 5) ∣ B :=
      dvd_trans (Nat.gcd_dvd_left d (B + 5)) hdB
    have h2 : Nat.gcd d (B + 5) ∣ (B + 5) := Nat.gcd_dvd_right d (B + 5)
    exact dvd_trans (Nat.dvd_gcd h1 h2) (gcd_B_C_dvd_five B)
  have hgcd1 : Nat.gcd d (B + 5) = 1 := by
    have h15 : Nat.gcd d (B + 5) = 1 ∨ Nat.gcd d (B + 5) = 5 :=
      (Nat.dvd_prime Nat.prime_five).1 hgcd5
    rcases h15 with h1 | h5eq
    · exact h1
    · have hd5 : 5 ∣ d := by
        have := Nat.gcd_dvd_left d (B + 5)
        rwa [h5eq] at this
      have h5B : 5 ∣ B := dvd_trans hd5 hdB
      exact absurd h5B h5n
  have hcop : Nat.Coprime d (B + 5) := hgcd1
  have hcop13 : Nat.Coprime d ((B + 5) ^ 13) := hcop.pow_right 13
  have hd1 : d ∣ 1 := by
    have := Nat.dvd_gcd (dvd_refl d) hdC13
    rwa [show Nat.gcd d ((B + 5) ^ 13) = 1 from hcop13] at this
  exact Nat.eq_one_of_dvd_one hd1

theorem gcd_A_B_eq_one_of_sol_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) : Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

theorem coprime_of_sol_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) : Nat.Coprime A B :=
  gcd_A_B_eq_one_of_sol h

/-- Remaining coprime branch after `3 ∤ B` and `5 ∤ B`.
    Coprimeness already holds for every gap-5 solution. -/
theorem coprime_of_remaining_gap5 {A B : ℕ}
    (h : is_gap5_sol A B) (_h3 : ¬ 3 ∣ B) (_h5 : ¬ 5 ∣ B) :
    Nat.gcd A B = 1 :=
  gcd_A_B_eq_one_of_sol h

/-! ## Linear form `Λ = 4 log A − 13 log(B+5)` -/

noncomputable def Lambda_gap5 (A B : ℕ) : ℝ :=
  4 * log (A : ℝ) - 13 * log ((B + 5 : ℕ) : ℝ)

theorem A_pow_four_eq_sub {A B : ℕ} (h : is_gap5_sol A B) :
    A ^ 4 = (B + 5) ^ 13 - B ^ 4 := by
  rw [← h, add_comm, Nat.add_sub_cancel_left]

theorem A_pow_real_add {A B : ℕ} (h : is_gap5_sol A B) :
    (A : ℝ) ^ 4 + (B : ℝ) ^ 4 = ((B + 5 : ℕ) : ℝ) ^ 13 := by
  have h' : ((A ^ 4 + B ^ 4 : ℕ) : ℝ) = (((B + 5) ^ 13 : ℕ) : ℝ) :=
    congrArg (fun n : ℕ => (n : ℝ)) h
  simpa [Nat.cast_add, Nat.cast_pow] using h'

theorem A_pow_pos_of_sol {A B : ℕ} (h : is_gap5_sol A B) :
    (0 : ℝ) < (A : ℝ) ^ 4 := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  exact pow_pos hApos 4

theorem Lambda_gap5_eq_log_div {A B : ℕ}
    (h : is_gap5_sol A B) :
    Lambda_gap5 A B =
      log ((A : ℝ) ^ 4 / ((B + 5 : ℕ) : ℝ) ^ 13) := by
  have hA : 0 < A := Nat.pos_of_ne_zero (A_ne_zero_of_sol h)
  have hApos : (0 : ℝ) < (A : ℝ) := by exact_mod_cast hA
  have hCpos : (0 : ℝ) < ((B + 5 : ℕ) : ℝ) := by
    have : 0 < B + 5 := Nat.add_pos_right B (by decide : 0 < 5)
    exact_mod_cast this
  unfold Lambda_gap5
  have h4 : (4 : ℝ) * log (A : ℝ) = log ((A : ℝ) ^ 4) :=
    (Real.log_pow (A : ℝ) 4).symm
  have h13 : (13 : ℝ) * log ((B + 5 : ℕ) : ℝ) =
      log (((B + 5 : ℕ) : ℝ) ^ 13) :=
    (Real.log_pow ((B + 5 : ℕ) : ℝ) 13).symm
  rw [h4, h13, Real.log_div (pow_pos hApos 4).ne' (pow_pos hCpos 13).ne']

theorem abs_Lambda_gap5_eq_log_one_plus {A B : ℕ}
    (h : is_gap5_sol A B) (hB : 0 < B) :
    |Lambda_gap5 A B| = log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) := by
  ...
  have hΛ := Lambda_gap5_eq_log_div h
...
theorem abs_Lambda_gap5_le_ratio {A B : ℕ}
    (h : is_gap5_sol A B) (hB : 0 < B) :
    |Lambda_gap5 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  have hA4pos := A_pow_pos_of_sol h
  have hBpos : (0 : ℝ) < (B : ℝ) := by exact_mod_cast hB
  have hu : (0 : ℝ) < (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
    div_pos (pow_pos hBpos 4) hA4pos
  ...
  have : log (1 + (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ≤
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
    simpa using hlog
  rwa [abs_Lambda_gap5_eq_log_one_plus h hB]

theorem B_pow_five_add_B_pow_four_lt_add_five_pow {B : ℕ} (_hB : 0 < B) :
    (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 5 : ℕ) : ℝ) ^ 13 := by
  have hx : (0 : ℝ) ≤ (B : ℝ) := Nat.cast_nonneg B
  have hC : ((B + 5 : ℕ) : ℝ) = (B : ℝ) + 5 := by
    rw [Nat.cast_add]
    norm_num
  have h5 : (B : ℝ) ^ 5 + (B : ℝ) ^ 4 < ((B + 5 : ℕ) : ℝ) ^ 5 := by
    rw [hC]
    have hexp : ((B : ℝ) + 5) ^ 5 =
        (B : ℝ) ^ 5 + 25 * (B : ℝ) ^ 4 + 250 * (B : ℝ) ^ 3 +
          1250 * (B : ℝ) ^ 2 + 3125 * (B : ℝ) + 3125 := by
      ring
    have h3 : (0 : ℝ) ≤ (B : ℝ) ^ 3 := pow_nonneg hx 3
    have h2 : (0 : ℝ) ≤ (B : ℝ) ^ 2 := sq_nonneg _
    nlinarith [hexp, h3, h2, hx]
  have hC1 : (1 : ℝ) ≤ ((B + 5 : ℕ) : ℝ) := by
    have : 1 ≤ B + 5 := le_trans (by decide : 1 ≤ 5) (Nat.le_add_left 5 B)
    exact_mod_cast this
  have h513 : ((B + 5 : ℕ) : ℝ) ^ 5 ≤ ((B + 5 : ℕ) : ℝ) ^ 13 :=
...
theorem A_pow_gt_B_pow_five {A B : ℕ}
  ...
  have hsum := B_pow_five_add_B_pow_four_lt_add_five_pow hB
  linarith [hA4, hsum]
...
theorem abs_Lambda_gap5_lt_inv_B {A B : ℕ}
    (h : is_gap5_sol A B) (hB : 0 < B) :
    |Lambda_gap5 A B| < 1 / (B : ℝ) :=
  lt_of_le_of_lt (abs_Lambda_gap5_le_ratio h hB) (ratio_lt_inv_B h hB)
...
theorem logA_gt_thirteen_of_B_ge_B0 {A B : ℕ}
    (h : is_gap5_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (13 : ℝ) < log (A : ℝ) := by
...
/-- `exp(−C1 log A log(B+5)) < B⁴/A⁴` for every `C1 ≥ 1` on
    a gap-5 solution with `B ≥ B0`. Same cutoff as `62383af`:
    `C1 < 1` would be needed. -/
theorem hGen_exp_lt_ratio_gap5
    {C1 : ℝ} (hC1 : 1 ≤ C1)
    {A B : ℕ} (hsol : is_gap5_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-C1 * log (A : ℝ) * log ((B + 5 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 := by
  set a : ℝ := log (A : ℝ)
  set c : ℝ := log ((B + 5 : ℕ) : ℝ)
  set C : ℝ := ((B + 5 : ℕ) : ℝ)
  ...
  have hCgt1 : 1 < B + 5 :=
    lt_of_lt_of_le (by decide : 1 < 5) (Nat.le_add_left 5 B)
  have hApos : (0 : ℝ) < (A : ℝ) := by
    have : (0 : ℕ) < A := lt_trans Nat.zero_lt_one hAgt1
    exact_mod_cast this
  have hCpos : (0 : ℝ) < C := by
    have : 0 < B + 5 := Nat.add_pos_right B (by decide : 0 < 5)
    have : (0 : ℝ) < ((B + 5 : ℕ) : ℝ) := by exact_mod_cast this
    simpa [C] using this
  have hCgt : (1 : ℝ) < C := by
    have : (1 : ℝ) < ((B + 5 : ℕ) : ℝ) := by exact_mod_cast hCgt1
    simpa [C] using this
...
theorem C1_floor_hGen_loses_gap5 {A B : ℕ}
    (hsol : is_gap5_sol A B) (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    exp (-(C1_floor : ℝ) * log (A : ℝ) * log ((B + 5 : ℕ) : ℝ)) <
      (B : ℝ) ^ 4 / (A : ℝ) ^ 4 :=
  hGen_exp_lt_ratio_gap5
    (le_of_lt (by
      simpa [C1_floor_eq_logs] using
        BealMatveevBeal.MatveevThreeLogs.C1_floor_gt_one))
    hsol hB hB0

/-! ## Floor form / LLL no-go (`B+5`) -/

noncomputable def floorFormThird_gap5 (C : ℝ) (A B : ℕ) : ℝ :=
  (4 : ℝ) * ⌊C * log (A : ℝ)⌋ - 13 * ⌊C * log ((B + 5 : ℕ) : ℝ)⌋

/-- `|4⌊C log A⌋ − 13⌊C log(B+5)⌋ − C Λ| < 17` for every real `C`. -/
theorem floor_form_approx_of_C_gap5 (C : ℝ) (A B : ℕ) :
    |floorFormThird_gap5 C A B - C * Lambda_gap5 A B| < 17 := by
  set a : ℝ := C * log (A : ℝ)
  set cval : ℝ := C * log ((B + 5 : ℕ) : ℝ)
  have hΛ : C * Lambda_gap5 A B = 4 * a - 13 * cval := by
    simp only [Lambda_gap5, a, cval]
    ring
  have hft : floorFormThird_gap5 C A B = (4 : ℝ) * ⌊a⌋ - 13 * ⌊cval⌋ := rfl
  have hform :
      floorFormThird_gap5 C A B - C * Lambda_gap5 A B =
...
theorem floor_form_third_abs_lt_gap5 {C : ℝ} (hC : 0 < C) (A B : ℕ) :
    |floorFormThird_gap5 C A B| < 17 + C * |Lambda_gap5 A B| := by
  have happ := floor_form_approx_of_C_gap5 C A B
  have hmul : |C * Lambda_gap5 A B| = C * |Lambda_gap5 A B| := by
...
/-- No scaling `C > 0` yields a Baker–Davenport witness
    `r > 17 + C/B0` on a gap-5 solution with `B ≥ B0`. -/
theorem hLLL_method_fails_gap5 :
    ∀ C : ℝ, 0 < C →
      ∀ A B : ℕ,
        is_gap5_sol A B →
          0 < B →
            B0_nat ≤ B →
              ¬ ∃ r : ℝ,
                17 + C / (B0_nat : ℝ) < r ∧
                  r ≤ |floorFormThird_gap5 C A B| := by
  intro C hC A B hsol hB hB0
  rintro ⟨r, hgt, hle⟩
  have htri := floor_form_third_abs_lt_gap5 hC A B
  have hLam := abs_Lambda_gap5_lt_inv_B hsol hB
  have hB0pos : (0 : ℝ) < (B0_nat : ℝ) := by
    have : (0 : ℕ) < B0_nat := by decide
    exact_mod_cast this
  have hBle : (B0_nat : ℝ) ≤ (B : ℝ) := Nat.cast_le.mpr hB0
  have hCΛ : C * |Lambda_gap5 A B| < C / (B0_nat : ℝ) := by
    have hmid : C * |Lambda_gap5 A B| < C * (1 / (B : ℝ)) :=
  ...
  have hthird : |floorFormThird_gap5 C A B| < 17 + C / (B0_nat : ℝ) := by
    linarith [htri, hCΛ]
  linarith [hgt, hle, hthird]

theorem no_uniform_c_cubic_in_N_gap5 :
    ¬ ∃ c : ℝ, 0 < c ∧
      ∀ L N1 N2 N3 : ℕ,
...
theorem coeffCount3_zero_ten_ten_ten_gap5 :
...
theorem no_uniform_c_easy_half_gap5 :
    (1 / 2 : ℝ) * 1 * 1 < 242 := by
  norm_num
...
/-- Matveev 2000 Theorem 1.4, n=2, displayed `C1_floor` exponential
    on a gap-5 solution. Not in Mathlib 4.12. Instantiating it at
...
/-- Bugeaud / LLL close for gap-5: ratio upper bound ⇒ `B ≤ B0`.
    The displayed lattice does not inhabit it. -/
def hLLL_gap5 : Prop :=
  (∀ A B : ℕ, is_gap5_sol A B → 0 < B →
      |Lambda_gap5 A B| ≤ (B : ℝ) ^ 4 / (A : ℝ) ^ 4) →
    baker_bound_gap5

def baker_bound_gap5_of_hGen_hLLL : Prop :=
  hGen_gap5 → hLLL_gap5 → baker_bound_gap5

def baker_bound_gap5_of_hGen_hLLL_gap5 : Prop :=
  baker_bound_gap5_of_hGen_hLLL
...
/-- Log and LLL methods lose on `B ≥ B0`. Remaining solutions are
    ...
    are impossible. Darmon–Merel `(4,4,13)` would be refuted by
    any such solution. `baker_bound_gap5` stays uninhabited as a
    named theorem. -/
theorem baker_bound_gap5_unconditional_nogo {A B : ℕ}
    (hsol : is_gap5_sol A B)
    (hB : 0 < B) (hB0 : B0_nat ≤ B) :
    (∀ C1 : ℝ, 1 ≤ C1 →
      exp (-C1 * log (A : ℝ) * log ((B + 5 : ℕ) : ℝ)) <
        (B : ℝ) ^ 4 / (A : ℝ) ^ 4) ∧
  ...
  refine ⟨?_, ?_, coprime_of_sol_gap5 hsol, ?_,
    B_even_of_sol_gap5 hsol, A_odd_of_sol_gap5 hsol,
    three_not_dvd_B_of_sol hsol, five_not_dvd_B_of_sol hsol, ?_⟩
  · intro C1 hC1
    exact hGen_exp_lt_ratio_gap5 hC1 hsol hB hB0
  · intro C hC
    exact hLLL_method_fails_gap5 C hC A B hsol hB hB0
  · exact Nat.lt_of_le_of_lt hB0 (A_gt_B_of_sol hsol hB)
  · intro hDM
    have hA0 : A = 0 ∨ B = 0 :=
      hDM A B (B + 5) (coprime_of_sol_gap5 hsol) hsol
    cases hA0 with
...
/-- Every gap-5 solution is impossible relative to
    `darmon_merel_4413_axiom`. Unlike gap-1 there is no `(1,0)`. -/
theorem no_gap5_of_axiom {A B : ℕ}
    (hsol : is_gap5_sol A B) : False := by
  ...
  | inr hB0 =>
      subst hB0
      exact five_not_dvd_B_of_sol hsol (by decide : 5 ∣ 0)

/-- Vacuous `B ≤ B0` for every gap-5 solution, relative to the
    existing Darmon–Merel axiom. Not named `baker_bound_gap5`. -/
theorem baker_bound_gap5_true {A B : ℕ}
    (hsol : is_gap5_sol A B) : B ≤ B0_nat :=
  (no_gap5_of_axiom hsol).elim

theorem baker_bound_gap5_true_of_axiom {A B : ℕ}
    (hsol : is_gap5_sol A B) : B ≤ B0_nat :=
  baker_bound_gap5_true hsol

#check C1_floor_eq
#check B0_nat_eq
#check not_gap5_one_zero
#check gcd_B_C_eq_gcd_five
#check gcd_B_C_dvd_five
#check gcd_A_B_eq_one_of_sol
#check coprime_of_sol_gap5
#check B_even_of_sol_gap5
#check A_odd_of_sol_gap5
#check A_gt_B_of_sol
#check no_sol_when_three_dvd_B_gap5
#check five_dvd_B_forces_five_dvd_A_gap5
#check no_sol_when_five_dvd_B_gap5
#check coprime_of_remaining_gap5
#check hGen_exp_lt_ratio_gap5
#check C1_floor_hGen_loses_gap5
#check floor_form_approx_of_C_gap5
#check hLLL_method_fails_gap5
#check no_uniform_c_cubic_in_N_gap5
#check no_uniform_c_easy_half_gap5
#check hGen_gap5
#check hLLL_gap5
#check baker_bound_gap5_of_hGen_hLLL
#check baker_bound_gap5_unconditional_nogo
#check baker_bound_gap5
#check no_gap5_of_axiom
#check baker_bound_gap5_true
#check baker_bound_gap5_true_of_axiom
#print axioms gcd_A_B_eq_one_of_sol
#print axioms B_even_of_sol_gap5
#print axioms no_sol_when_three_dvd_B_gap5
#print axioms no_sol_when_five_dvd_B_gap5
#print axioms five_dvd_B_forces_five_dvd_A_gap5
#print axioms hGen_exp_lt_ratio_gap5
#print axioms C1_floor_hGen_loses_gap5
#print axioms hLLL_method_fails_gap5
#print axioms baker_bound_gap5_unconditional_nogo
#print axioms no_gap5_of_axiom
#print axioms baker_bound_gap5_true

end BealMatveevBeal.BealGap5
